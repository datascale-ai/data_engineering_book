...
## 6. コード拡張 1: 上流のプロジェクト資産を要約する

`src/collect_upstream_projects.py` 上流のプロジェクト資産を要約し、プロジェクト情報を統一仕様に整理する責任を負います。次のコード スニペットは、レジストリの中心的な構造を示しています。```python
PROJECT_SPECS = [
    {
        "project_id": "p1",
...
* 「プロジェクトが存在する」ということは、「プロジェクトは下流で使用できる」ということと同じではありません。
* フライホイールの最初のステップは、プロジェクト ディレクトリを構造化されたアセット ディレクトリに変えることです。
...
パイプラインはシーケンスを重視するのに対し、フェージングは次のことを重視するためです。

* 現在の目標は何ですか。
* ステージ出力とは何ですか。
* 次の段階に進むためのしきい値は何ですか。
* この期間において主な責任を負うリソースとチーム。
...
`src/build_flywheel.py` は、最初の 9 つの項目をフライホイール構造にマッピングする役割を果たします。次のコード スニペットは、5 層アーキテクチャの構造化表現を示しています。```python
...
`src/evaluate_flywheel.py` 负责把散落在多份产物中的结果收束成系统级指标与总报告。下面的代码片段展示了这种计算方式。```python
...
* テキストでは、システムレベルの指標の工学的重要性を説明する必要があります。
...
* コマンドレベルのチェック項目 `2`;
* データ/製品レベルのチェック項目 `11`;
* コマンドレベルの範囲 `py_compile, evaluate_flywheel`;
...
なぜなら、システムプロジェクトは「すべての部分は正しいが、全体が正しくない」という問題が最も発生しやすいからです。たとえば:

* JSON ファイルは存在しますが、フィールドはレポートと一致しません。
* 特定のフェーズの計画はよく書かれていますが、マイルストーンは同期的に更新されません。
* コードは機能しますが、レポート全体は依然として古いデータを参照しています。
※ガバナンスバウンダリーはプロジェクトにより補完されているが、検査項目の対象外となっている。
...
P10 の `src/run_p10_checks.py` スクリプトは、最終アセンブリ層の受け入れルールを実行可能なエンジニアリング契約に書き込みます。次のコード スニペットは、検査スクリプトの基本構造を示しています。```python
