...
- `include_tables=False`: テーブル レイアウトによって引き起こされる構造上のノイズを軽減します。
- `no_fallback=False`: 再現率を向上させるために必要な場合、抽出コンポーネントが修復抽出を実行できるようにします。
...
> 実際のWebページデータにおいて、「テキストっぽい」候補テキストをどれくらい安定して取得できるでしょうか？

---

## 6. ヒューリスティック クリーニング: ノイズ スクリーニングの最初のラウンド
...
### 6.2 このプロジェクトで採用された主な清掃ルール

#### 1) 長さのルール

- 短すぎるテキストは破棄します。 100文字未満
...
> 高価なモデルのスコアリングに依存せずに、最初に最も粗いノイズを抑制して、その後のより細かい処理のためのリソースを節約します。

---

## 7. 重複の削除: Web コーパスにおける重複に近い処理
...
1. 特定のコンテンツが過度に繰り返され、配信が不均衡になります。
2. モデルのメモリが特定のテンプレートまたはサイトに対して強すぎる可能性があります。
3. その後の評価中にデータ漏洩が発生する可能性があります。
4. 重複したコンテンツによってストレージとトレーニングのリソースが不必要に消費されます。
...
- **LSH (ローカル機密ハッシュ)**: 類似したテキストが同じ候補バケットに分類されやすくなり、グローバル比較の数が減ります。
