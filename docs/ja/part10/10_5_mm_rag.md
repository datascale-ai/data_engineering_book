...
> そもそも信頼性が低い場合、システムは複数ページの証拠を使用して調べることができますか?

この観点から見ると、複数ページのリコールはあれば便利というわけではなく、必要な堅牢性のメカニズムです。

### 9.4 実行可能なさらなるフィルタリングロジック

ディレクトリ ページのフィルタリングに加えて、次のことも考慮できます。

* 情報密度の低いページのフィルタリング
* 純粋な著作権ステートメントまたは表紙ページのフィルタリング
* 重複したページを削除します
*クエリタイプと一致しないページタイプのフィルタリング
...
### 10.2 マルチモーダル シーンがノイズによって妨げられる可能性が高い理由
...
## 11. マルチグラフのコンテキスト構成: マルチグラフの証拠の整理
...
* このタスクの目標を明確にするために、質問テキストを最初に入力する必要があります。
* 画像の順序は可能な限り安定している必要があります。
* ページ番号がある場合は、各写真がどのページに対応するかをモデルに知らせるのが最善です。
* ページがノイズの多いページである可能性がある場合は、テキスト内でモデルに注意して扱うように注意する必要があります。
* 出力にページ番号またはソース ページを引用することを要求することが最善です。
...
## 12. ステップバイステップの実践: インデックス作成から質問と回答までの再現可能な最小のリンク

この部分は、プロジェクトの既存の実装アイデアに従い、リンクを再現可能なエンジニアリング プロセスに編成する方法に焦点を当てます。

### 12.1 フェーズ 1: 視覚的なインデックスの構築

現在の実装では、プロジェクトは Byaldi を通じて ColPali をカプセル化し、PDF ページを視覚的にエンコードし、元の画像をインデックスとともに保存します。このステップの鍵は、「数行のコードを記述する」ことではなく、後続のシステムが同じページ画像を安定して取得できるようにすることです。```python
import os
...
既存の実装では、`RETRIEVAL_K` を 4 (実用的なデフォルト) に設定します。マルチモーダル入力を過度に拡大することなく、一定の証拠範囲を提供できます。```python
RAG = RAGMultiModalModel.from_index(INDEX_NAME)
RETRIEVAL_K = 4
results = RAG.search(user_query, k=RETRIEVAL_K)
```### 12.3 フェーズ 3: マルチグラフ推論

既存の実装では、質問テキストと複数のページ画像を組み合わせてペイロードを形成し、それを処理のために Qwen2.5-VL に送信します。重要なポイントは 2 つあります。1 つは、ディレクトリを無視するようにモデルに明示的に要求することです。もう 1 つは、財務報告書の小さな文字や密な数字を読み取るために、画像の詳細を `high` に設定することです。```python
content_payload = [{
    "type": "text",
    "text": (
...
* 現在のページは傾向の判断を裏付けることができますが、正確な数値的結論を裏付けるには十分ではありません。
* この質問には複数のエンティティまたは複数の年が関係しており、さらなる検証が推奨されます。
* 目次ページまたは概要ページがリコールされており、現在の結論を裏付けるためにさらに多くのテキスト ページが必要になる可能性があります。
