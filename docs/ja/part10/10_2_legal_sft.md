...
### 6.1 最初のシードとしての規則テキスト

* 構造は比較的明確であり、切断に適しています。
* 比較的高い権限があり、基本的な監督に適しています。
※練習方法の説明や知識Q&A、ルールのまとめに便利です。
* 小規模プロジェクトでの品質管理の達成が容易になります。
...
## 7. PDF の解析とインテリジェントなクリーニング: 法的文書のフォーマット クリーニング
...
※本来は別々の文節を一つの文にまとめています。
※法律記事の本来の連続ロジックは、ページ番号、見出し、単語区切りなどによって断片化されています。
...
|アレンジロジック | `Python` |バッチ処理、サンプリング、後処理 |再現可能な最小プロセスを迅速に構築するのに便利 |

### 7.3 ヘッダーとフッターをトリミングする
...
※抽出後の洗浄よりも発生源からのノイズが軽減されるため安定しています。
* ほとんどの規制 PDF への優れた適応性。
* 実装が簡単で、最小限の再現可能なソリューションとして適しています。
...
ヘッダーやフッターと比較すると、より微妙な問題は、テキストに埋め込まれたページ番号です。たとえば、次のとおりです。```text
……应当依法承担相应责任。 - 195 - 当事人……
...
>法文に微妙な修正を加えているのでしょうか、それとも法文自体を損なうために粗雑なルールを使用しているのでしょうか？

### 7.5 中国語の単語の分割を修復する

中国語の PDF に関するもう 1 つの一般的な問題は、次のような「誤ったスペース」です。```text
法 律 规 定
合 同 关 系
...
如果让一个团队凭直觉去做法律 SFT，最容易得到的数据集往往是这样的：
...
* 法的シナリオにおけるリスク境界は、医学的または経済的境界と同等ではありません。
* 法解釈業務は他の業界では主要な業務ではない場合があります。
...
> 信頼できるシードを見つける -> 構造化されたダイシングを行う -> タスク システムを設計する -> 制御された合成拡張 -> QA と設定を確立する -> リスク境界を個別にモデル化する -> トレーニングのカプセル化と一貫性の検証。
...
* 法律条項、判例の概要、リスク警告の引用に明らかな誤りや古い表現があるかどうか。
* 拒否サンプルと高リスク相談サンプルが主要な境界をカバーしているかどうか。
* 設定ペアと QA レコードが現在のバージョンの結論を本当に裏付けているかどうか。
