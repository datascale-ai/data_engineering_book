# Data Engineering for Large Models

## Current Edition Status

The Chinese 2026 edition is the mainline edition of this book. It now covers 28 chapters and 10 runnable project chapters.

The English edition is being updated and is not yet synchronized with the latest Chinese table of contents. To avoid presenting an outdated outline as current, this page now serves as a status entry.

- [Preface](preface.md)
- [Chinese 2026 Edition Status](translation-status.md)
- [Latest Chinese Table of Contents](../)

## Latest Chinese Scope

- Part 1: Overview and Infrastructure
- Part 2: Text Pre-training Data Engineering
- Part 3: Multimodal Data Engineering
- Part 4: Instruction Fine-tuning and Preference Data
- Part 5: Synthetic Data Engineering
- Part 6: Reasoning and Agent Data Engineering
- Part 7: Application-Level Data Engineering
- Part 8: Data Operations and Platform Development
- Part 9: Privacy Compliance and Data Security
- Part 10: Practical Projects
