...
* Seed tasks and course structure design: organize math questions, code questions and chapter gradients.
...
![Figure 1: Positioning map of the Synthetic Mathematics and Code Textbook Factory Project](../../images/part10/10_4_fig01_project_positioning.png)
...
> Starting from GSM8K and MBPP style seed questions, rewrite math questions and coding questions into more complete curriculum samples, and stably precipitate trainable teaching material assets through program execution and inspection scripts.
...
![Figure 2: Schematic diagram of P04 project goals and boundaries](../../images/part10/10_4_fig02_goals_and_scope.png)
...
* Whether data assets can be directly entered into training or are only suitable for display.
...
* Why should textbook content include volumes and course maps instead of scattered questions and answers;
* Why quality control should be carried out before data production, rather than remediation after training;
...
![Figure 3: P04 overall architecture overview](../../images/part10/10_4_fig03_pipeline_overview.png)
...
### 4.2 The second layer: evolutionary generation and PoT construction layer

What this layer solves is "how to turn the questions into teaching materials with more training value." Mainly include:

* Rewrite the original question in Evol-Instruct style;
*Introduce scenarios, constraints and multi-step reasoning;
* Let the model output Python code instead of just giving the natural language thinking process;
...
* Clean low-quality samples and record the reasons for failure;
* Packaged into textbook volumes, curriculum maps, teacher guides and training documents;
* Confirm product consistency by checking scripts.
...
## 5. Project pre-production: key aspects of the teaching material factory

![Figure 4: Responsibility coordination diagram of teaching material factory](../../images/part10/10_4_fig04_roles_and_responsibilities.png)
...
* What questions can better reflect advanced reasoning;
...
* Whether the intermediate products are traceable.
...
* The metrics look good, but no one knows which layer the problem is originating from.
...
![Figure 5: Mapping diagram from seed questions to chapter plan](../../images/part10/10_4_fig05_seed_to_plan.png)
...
* **Field Coverage**: Both mathematics and coding are required;
...
![Figure 6: Evol-Instruct evolution path diagram](../../images/part10/10_4_fig06_evol_path.png)
...
3. Carry out scene-based rewriting;
...
* Make subsequent PoT more like "solving real problems" rather than "setting up formulas".
...
### 7.3 Why does “increasing difficulty” also emphasize “maintaining solvability”?
...
* Adding too many conditions to the question eventually becomes inconsistent or has no unique solution.
...
## 8. PoT selection: programmatic reasoning path

![Figure 7: CoT and PoT comparison chart](../../images/part10/10_4_fig07_cot_vs_pot.png)
...
* How to return from program results to natural language conclusions.
...
## 9. Generate link: from prompt to specific implementation of code solution

![Figure 8: Generate link refinement graph](../../images/part10/10_4_fig08_generation_chain.png)
...
### 9.4 Why is retrying on failure important?
...
![Figure 9: Sandbox verification execution path diagram](../../images/part10/10_4_fig09_sandbox_validation.png)
...
### 10.1 What happens if you don’t verify it?
...
* Brutal exhaustion causes runtime to get out of control.
...
Model output is often irregular. Sometimes it is wrapped in ```python, and sometimes it is only plain ```. If the extraction logic is not compatible, the system will misjudge many otherwise usable code snippets as empty output. The original chapter clearly gives the two-level matching logic of `extract_python_code()`. This is a typical engineering detail: not flashy, but very critical.
...
> Teaching materials without verification are more like beautifully packaged text; teaching materials with verification are closer to trainable assets.

---

## 11. Teaching material packaging: curriculum asset organization

![Figure 10: Textbook packaging product relationship diagram](../../images/part10/10_4_fig10_packaging_outputs.png)
...
### 11.1 Textbooks are not a pile of samples
...
* How to divide mathematics and coding content into papers;
* How to gradually increase the difficulty;
* Which questions are suitable for smoke tests;
* How the teacher or reviewer can quickly understand the entire set of materials.
...
* Help maintain a consistent structure when expanding new volumes in the future.

### 11.4 Why the Teacher’s Guide is not the “icing on the cake”
...
![Figure 11: Training package interface diagram](../../images/part10/10_4_fig11_training_interface.png)
...
* Whether the sample schema is unified;
...
### 12.2 Training assets of the current project
...
* Whether the training script can read data;
...
* At the current scale, verify that the link is stable;
...
### 13.3 What does topic distribution indicate?
...
### 13.4 What does difficulty distribution indicate?
...
* On the other hand, it also means that if it is to target a wider range of learners in the future, basic questions and transitional questions need to be supplemented.

---

## 14. Engineering effect: core problems solved by P04
...
### 14.2 Solved the problem of "difficult to organize teaching material content"
...
## 15. Cost and optimization: bottleneck before scale expansion
...
* The generation takes a long time and the throughput is insufficient;
* Performing verification serialization, the overall batch running speed is slow;
* Rework and retry of failed samples will lengthen the total link.

### 15.3 How can it be expanded?

Combined with the current implementation, future expansion can be prioritized in the following directions:

1. **Concurrency generation**: Change the single-thread loop to a thread pool, asynchronous request or task queue;
2. **Task Distribution**: Use `Celery + RabbitMQ` or a lighter worker to distribute and generate tasks;
...
### 15.4 Why is the focus of optimization here not “making it cheaper”?
...
* Validation and quality documentation: `verified_textbook.jsonl`, `verification_failures.jsonl`, `execution_results.jsonl`, `quality_audit.jsonl`
* Course organization documents: `curriculum_map.json`, `textbook_catalog.json`, `editorial_style_guide.md`
...
## Special topic: Acceptance standards of teaching material factories
...
* The question is complete, no known conditions are missing, and no key units, variable ranges or input and output constraints are hidden in fuzzy expressions;
* The solution should be consistent with the question. There should be no situation where the question is calculating the total price but the code is calculating the average, or the question has been rewritten but the program still uses the old logic;
* The program output is consistent with the final answer and is re-verifiable under the current question type;
* Explanation text can help learning, rather than just writing synonyms outside the code;
...
* Whether the chapter dependencies are clear, and the skills used in the following chapters should at least have foreshadowing or minimal introduction in the previous chapters;
...
* The first appearance of basic concepts in the book;
...
### 3. Training layer acceptance: Whether the teaching material assets can stably enter the training
...
* Whether the responsibilities of the three sets of train, val, and smoke data are clear, and there will be no leakage of the verification set or the smoke set degenerating into a display sample;
* Whether both subject areas are divided into segments to avoid training that only covers mathematics or only code;
...
## Special topic: Operation mechanism of teaching material factory
...
* Complete manual inspection, volume packaging and training packaging in the second half of the week;
...
### 3. Failure rework: turn bad samples into improvement clues
...
* Whether there are problems with verbose language, erratic style or inconsistent terminology;
...
* Verifier changes, for example, physics questions may rely more on unit analysis and numerical simulation, and statistics questions may rely more on data sampling and distribution testing;
* Changes in curriculum structure, for example, programming textbooks place more emphasis on project drive, and mathematics textbooks place more emphasis on conceptual progression;
* Changes in expression style, for example, algorithm questions focus more on complexity analysis, and data analysis questions focus more on experimental explanation.
...
* Which explanations are most likely to cause misunderstandings;
...
* Priority will be given to which gaps will be filled in the next version.
...
* Explain whether the density is appropriate;
* Whether the terminology within the chapter is consistent;
* Whether the summary after the question really plays a teaching role;
