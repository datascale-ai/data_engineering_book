...
In many projects, "why these fields should be protected" will be implicit in code comments. But P09 makes this explicit as `compliance_scope.json`.这样做有三个价值：
...
The corresponding code is as follows:```python
...
`build_classification_policy()` organizes these rules into structured objects:```python
...
`build_raw_records()` in `run_privacy_pipeline.py` gives representative data directly:```python
...
A truly robust privacy classification is often not about "looking only at field content" or "only looking at data sources", but a combination of the two. P09's `classify_record()` reflects this.```python
...
P09 Three strategies are used in `redact_payload()`: tokenize, mask and remove.```python
...
Use `sha256` in the auxiliary script to generate stable tokens:```python
...
P09 explicitly gives four types of zones through `build_isolation_plan()`: raw_zone, quartine_zone, redacted_zone and audit_zone.```python
...
`build_alerts()` constructs two typical alarms:```python
...
P09 In `simulate_privacy_ops.py`, do preflight first and then do incident simulation. This sequence is very engineering.```python
preflight = {
    "checks": [
...
P09 Write incident scenarios as structured records: The analyst attempted to export restricted raw records without approval, where detection, containment, outcome, and response_minutes were all explicitly retained.```python
incident = {
...
The evaluation phase will first read all products such as scope, classification, access, tech options, raw/classified/redacted/quarantined, alerts, audit, preflight, incident, postmortem, etc., and then calculate the key results.```python
metrics = {
