...
There is also a category of information that should not enter memory at all, nor even be retained in state for long: information that has been explicitly negated by the user, one-off conversational content, unconfirmed speculation, and low-value details that will not help in the future. For data teams, the difficult discipline lies in rigorously refusing to record what should not be recorded, not only in finding what can be recorded. Especially in multi-turn task-oriented agents, the better the system's apparent "memory," the more important it is to restrain the impulse to write. Once unconfirmed speculation, casual verbal expressions, or one-time preferences are written into the long-term layer, every future confident retrieval of that content will amplify the error into a structural bias.
