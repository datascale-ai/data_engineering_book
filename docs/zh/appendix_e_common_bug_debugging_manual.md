...
本附录面向数据工程里最常见、也最消耗时间的一类问题：数据管线“看起来都对”，但结果就是不对。它不是一份通用故障库，而是一份更接近本书语境的调试手册，帮助读者把常见问题从“依赖经验排查”转变为“可定位、可归类、可回归测试”的对象。
...
如果采集的是长文档、公告、论文或知识库页面，还要额外检查分页加载和懒加载内容。很多“缺字”并不是没抓到，而是页面只在滚动后才渲染。此时应把“截图”与“DOM”同时保存，避免只看其中一种证据。
...
如果一个问题修了两次还会回来，通常不是技术细节问题，而是记录不完整。调试记录的价值不只是“留档”，更是给后续类似问题建立路径依赖，让团队少走重复路。对于需要进入生产或课程复现的项目，类似 ML Test Score 的检查框架也提醒我们：测试、监控和可回滚能力本身就是系统成熟度的一部分（Breck et al. 2017）。
...
标准化不是为了形式统一，而是为了降低认知负担。每次都按同一顺序看，团队成员之间更容易共享判断，也更容易在交接时承接问题。站点可靠性工程的实践同样强调，将故障响应、监控、复盘和自动化运维沉淀为标准动作，才能降低重复事故的长期成本（Google SRE 2016）。
...
调试手册真正有价值的地方，不是告诉你某个 Bug 怎么修，而是帮助团队形成一套稳定的判断方式。这样做之后，排障不再依赖“某个特别会修的人”，而是逐渐变成可复制的工程能力。

## E.23 20 类常见问题清单

为了贴近真实项目，这里把常见问题再压实成 20 类。它们不是互斥的，但能帮助你快速归位。
