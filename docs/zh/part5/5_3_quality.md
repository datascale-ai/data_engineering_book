...
本章并不是要否定合成数据，而是讨论它在工程中可能带来的副作用。合成数据确实能提高效率、补充长尾样本，也能降低一部分标注成本；但如果缺少治理，它也可能带来风格单一、知识漂移、样本越来越相似、训练表面稳定但线上变差等问题。对负责生成、质检、训练配比和上线治理的团队来说，关键不在于“用不用合成数据”，而在于怎样控制它的比例、来源和验证方式，让它持续提供增益，而不是把风险一起放大。

---

## 17.1 为什么合成数据会反噬模型能力

合成数据的风险并不主要来自“质量一定差”。很多合成样本本身可能写得很规范，格式也很整齐。真正麻烦的是，当生成、筛选和训练都在同一套逻辑里循环时，系统会越来越偏向自己熟悉的数据形式。时间久了，问题不一定表现为某一批样本明显错误，而是整个训练分布慢慢远离真实任务。
...
问题在于，合成数据用得多了以后，就不再只是补充材料，而会开始主导训练分布。生成器、裁判器、模板系统和训练器如果长期绑在一起，样本空间就会越来越受系统内部偏好影响。只要生成器、裁判器、模板系统和训练器之间形成紧密闭环，样本空间就会越来越受系统内部偏好牵引，而不再主要受真实世界牵引。于是，模型表面上是在不断吸收“新数据”，实际上却是在反复吸收“同一套生成逻辑的不同表面变体”。这种变化通常不会立刻制造大量明显错误，而是会慢慢改变模型擅长处理的问题类型。模型会越来越适应被模板整理过的问题，却越来越不适应真实世界里那些不完整、不规整、边界模糊的输入。
...
对很多团队来说，自举最吸引人的地方，不只是能多生成一些数据，而是它明显加快了数据生产节奏。过去一条样本要经过需求拆解、人工撰写、审核和入库，自举跑起来后，很多环节都能被模型先顶上。过去一条样本从需求提出、任务拆解、人工撰写、审核验收到最终入库，链路很长，产能也很容易卡在人工环节上。自举一旦跑起来，整个系统就像突然多了一台能够持续供料的机器。模板可以快速扩展，长尾任务可以先用教师生成一个初版，原本因为成本太高而迟迟补不上的边界场景，也可以先通过模型模拟出来。对项目处在快速爬坡期的团队来说，这种产能释放几乎是立竿见影的。
...
所以，自举一开始就同时带着收益和风险。早期收益更明显，风险不容易被看到；后期合成比例上来以后，风险才会慢慢变成结构性问题。只是早期收益更显眼，风险更隐蔽；到了后期，风险逐渐积累成结构问题，收益却未必还能像前期那样持续放大。团队不应该只问“自举好不好”，而要持续观察它的作用边界：它是在补充真实数据，还是已经开始替代真实数据；它是在放大有效监督，还是在放大系统内部偏好。
### 自举为什么在早期看上去总是有效
...
自举真正麻烦的地方，不是模型参与生成样本这件事本身，而是整个系统容易越跑越封闭。教师模型负责生成，裁判模型负责筛选，模板负责统一结构，学生模型再学习这些结果。几轮之后，系统会越来越熟悉自己的偏好。

这种封闭不代表系统不变化。它可能仍然在快速生成、筛选和训练，只是变化越来越发生在内部规则里，而不是由真实世界推动。模型学会了什么样的表达容易被保留，什么样的结构最容易通过裁判，什么样的答案最符合模板，于是下一轮生成自然更倾向于这些形式。久而久之，系统不再是围绕现实世界的多样性来塑形，而是围绕自身已经形成的一套偏好来打磨自己。它还在学习，但学习对象越来越多地变成了“自己认可过的东西”。
...
所以，风格坍缩不能只看句子有没有重复。更重要的是，模型是不是越来越习惯用同一种方式理解问题、组织答案和处理边界。它不是完全不会换说法，而是越来越少主动换处理思路。
...
知识漂移最难发现的地方在于，它一开始不一定表现为事实错误，而是先体现在解释方式上。模型可能还知道相关事实，但越来越喜欢用固定框架来解释问题。也就是说，模型的“知识表象”可能暂时没有坏，但它的知识使用方式已经开始变化。
