...
因此，讨论合成数据时，重点不应只放在 prompt 怎么写，而应放在生成过程能否被复用、评估和治理。一个可用的合成数据系统，不能只负责生成样本，还需要覆盖任务设计、模板编排、候选扩展、质量筛选、难度控制、去重去污染、人工抽检、版本追踪和反馈迭代等环节。只有进入这样的流水线，合成数据才可能稳定地服务后续训练。
...
合成数据进入大规模使用后，团队需要从单次生成转向稳定生产。这里的生产，不是简单增加模型调用次数，而是要建立一套完整流程：原料输入、模板装配、任务调度、多模型协作、质量闸门、返工回流、版本管理和日常监控都要能够接上。这样，合成数据才不只是一次性生成结果，而能持续服务训练、评估和产品迭代。
...
传统数据增强更多是在已有样本上做变形，合成数据工程则是在重新组织一条数据生产链。它要处理的不只是生成方法，还包括任务选择、生成模型、模板设计、裁判机制、合格标准和失败回流。也就是说，合成数据不能只作为一个 prompt 技巧来看，它会同时影响数据设计、模型调用、质量控制和训练运营。
...
如果把合成数据理解成“自动写文本”，会低估它的工程难度。对训练来说，模型生成的内容本质上是在提供监督信号。它有没有价值，不只看语言是否通顺，还要看它是否准确表达了团队希望模型学习的行为模式。比如在工具调用任务里，真正关键的不是回答写得是否自然，而是模型是否学会在正确时机选择正确工具、填入正确参数并处理正确返回值；在安全拒答任务里，关键也不是拒答语句是否礼貌，而是模型是否在风险边界上形成稳定而一致的行为。
...
因此，多模型协作的重点不在模型数量，而在职责拆分。生产者负责生成候选样本，批评者负责发现问题，裁判者负责是否放行，验证者负责出厂检查，人工负责校准标准。职责拆开后，系统才更容易同时控制产量和质量。
...
合成数据的难点不在于让模型生成内容，而在于让生成过程长期稳定运行。本章围绕这一问题，讨论了零散 prompt 为什么难以形成稳定产能，合成数据工程为什么不同于传统数据增强，以及成本、质量和吞吐为什么需要放在同一个系统中权衡。进一步地，我们从种子池与模板库出发，说明了高质量 seed 如何决定工厂的原料质量，模板库如何把少量精品样本扩张为大规模且可控的样本体系。
