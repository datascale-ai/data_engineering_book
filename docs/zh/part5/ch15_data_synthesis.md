...
但这条路径并不天然安全。模型制造数据，看似提高了效率，实则也带来了新的系统性风险：模型可能放大原有偏差，重复生成低信息密度内容，制造表面正确却本质脆弱的样本，甚至在自我循环中不断“喂养”自身缺陷，最终形成数据污染、能力塌缩或错误行为固化的反噬效应；递归使用模型生成数据导致模型塌缩或分布尾部丢失的风险，已经在相关研究中被系统讨论 (Alemohammad et al. 2024; Shumailov et al. 2024)。也就是说，合成数据的关键问题始终在于：怎样生成得可控、可信、可验证、可持续。
...
这个失败并不是某一个 prompt 写得不好，也不是某一批数据清洗得不够干净，而是合成数据生产中更深层的系统性风险：当生成、筛选、训练之间缺少外部校验和分布约束时，数据流水线会把模型已有的偏差重新包装成“新数据”，再通过训练过程固化进下一代模型。短期看，数据规模增加了；长期看，真实分布被稀释了，罕见模式被抹平了，模型能力也可能在自我循环中退化。因此，合成数据工厂首先要解决的并不是“如何让模型生成更多数据”，而是“如何防止模型用看似合理的数据不断喂养自己的缺陷”。这也是本章讨论合成数据生产流水线的起点：合成数据只有被纳入任务设计、质量评估、分布控制、人工抽检和版本追踪等完整机制之中，才可能从一种便捷的生成技巧，转化为可靠的训练基础设施。
...
self-play 则是多模型协同中的一种特殊形态。它尤其适合多轮对话、辩论、协商、红蓝对抗和复杂工具使用场景。比如，一个模型扮演用户，不断提出更刁钻的问题；另一个模型扮演助手，尝试回应；第三个模型负责判断对话是否具有训练价值。这样做的优势，是能生成大量单轮模板不容易覆盖的交互轨迹。Self-play fine-tuning（SPIN） (Z. Chen et al. 2024)和 Constitutional AI (Bai et al. 2022) 等工作，都利用模型反馈、自我博弈或 AI feedback 来减少对额外人工标注的依赖。但 self-play 的风险也同样明显：如果缺少真实种子锚定和质量闸门，它很容易进入自我循环，生成越来越“像模型想象中的难题”，而不是现实世界中的难题。
...
分层闸门的价值，在于它能让问题在最早出现的地方被识别。例如，格式问题根本不该进入语义裁判层；角色漂移问题应尽量在规范闸门就被捕捉；多样性不足属于批次层面的问题，应由批次级闸门处理，不能归因于单条样本有错。把所有判断压到最后一层，只会让工厂越来越像黑箱。分层闸门虽然设计更复杂，但它换来的是可解释的质量治理结构。
