...
| **大模型数据工程师** | 原始语料采集（爬虫/应用程序编程接口（Application Programming Interface，API））、多阶段清洗（去重、去噪、脱敏）、数据配比与混合采样、数据版本管理 | 算法团队的领域权重配比需求、安全合规的黑名单规则、标注团队的 SFT 样本反馈 | 通过质量评分卡验收的 Parquet/JSONL 格式数据包；数据血缘文档 | 每批次数据包的清洁度评分 ≥ 0.85；交付 SLA：提出需求后 T+3 个工作日内完成新语料接入 |
