...
- [第47章：LLM 预训练数据工程实战：从配方到落地](ch47_pretrain_recipes.md)
- [第48章：LLM 后训练数据工程实战：SFT 与偏好对齐](ch48_posttrain_recipes.md)
- [第49章：推理模型与 RL 数据工程：R1 / QwQ 范式](ch49_rl_reasoning_data.md)
- [第50章：多模态大模型（VLM）数据配方：从预训练到视觉对齐](ch50_vlm_data_recipes.md)
- [第51章：多模态生成模型数据工程：T2I 与 T2V 数据流水线](ch51_t2i_t2v.md)
...
建议先读第47章和第48章，建立预训练与后训练的数据配方基线；再读第49章，理解推理模型与 RL 数据工程的分化；最后阅读第50章和第51章，进入 VLM 与多模态生成模型的数据配方。
