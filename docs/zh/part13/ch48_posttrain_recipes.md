...
## 48.0 开篇问题场景：为什么 Alpaca 训不出 GPT-4 风格
...
## 48.1 后训练数据的三段论：SFT、偏好对齐与在线持续优化
...
![图48-1：LLM 后训练三阶段流水线示意图](../../images/part11/30_1_posttrain_three_stage_pipeline.png)
*图48-1：SFT、偏好对齐与在线持续优化之间的数据流动关系。*
...
表48-1 中的 `[D]` 表示公开材料直接披露的信息，`[I]` 表示依据公开流程或上下文做出的合理推断。无法直接追溯的数字不应写成确定规模，而应保留为“未披露”或“需按来源核验”。
...
## 48.3 SFT 数据合成三大流派：Self-Instruct、Evol-Instruct 与 Magpie
...
### 48.3.1 Self-Instruct：从种子任务扩展指令空间
...
### 48.3.2 Evol-Instruct：通过进化规则提升复杂度
...
![图48-2：Self-Instruct、Evol-Instruct 与 Magpie 三流派 pipeline 对比](../../images/part11/30_2_sft_synthesis_pipelines.png)
*图48-2：三类 SFT 合成路线的数据入口、生成方式与质检位置。*
...
## 48.4 偏好数据工程：从 RLHF 到 DPO、GRPO 与 RLVR
...
![图48-3：Tülu-3 三阶段数据流与规模示意](../../images/part11/30_3_tulu3_posttrain_flow.png)
*图48-3：Tülu-3 从 SFT-Mix、DPO mix 到 RLVR 的数据流与阶段关系。*
