...
本章也不展开视频生成模型架构，不讨论 DiT、U-Net、Transformer、Flow Matching 或 VAE 的细节；多模态理解任务由第47章以及第三篇的基础章节负责。这里关心的是另一类更隐蔽的工程能力：如何让原始媒体变成模型真正能学会的生成监督。
...
本章的边界也要守住。通用视频切片、ASR、说话人分离、字幕时间轴这些内容已经在第十章处理；多模态理解、视频问答和事件检测由第47章以及第三篇的基础章节接管；本章只讨论生成模型训练数据如何被过滤、重写、路由和审计。这个边界能避免章节重复，也能让读者看到生成数据工程真正新增的部分。
