...
这意味着，系统设计不能只围绕“功能是否能验证”，还必须围绕“地区规则差异如何映射到产品、数据和模型链路”。例如，同样是用户画像，在不同法域下可能对应不同的法律依据、保留期限、用户权利响应机制和共享限制。真正成熟的治理体系，不是让开发者去背诵所有法规条文，而是把这些差异沉淀到模板、策略、审批规则和默认行为中(Zieni et al. 2021; Kosenkov et al. 2026)。

### 36.1.3 治理交汇点：数据、模型与业务的协同
...
如果这三者分离，系统就会出现常见错位：业务要求新能力，模型迅速接入新特征，平台侧开放了可用接口，但没有人统一判断这条数据链路是否合规。只有通过统一的元数据管理中心、策略中心和审计中心，才能把这些原本散落在不同团队的要求联动起来。图36-1展示了合规左移与治理协同的整体架构，说明合规审查如何从上线前置到需求分析与架构设计阶段。

![图36-1：合规左移与治理协同架构图](../../images/part11/图36_1_合规左移与治理协同架构图.svg)
*图36-1：合规左移策略 —— 展示合规审查如何从上线前置到需求分析与架构设计阶段*
...
### 36.2.4 风险评估矩阵（Risk Assessment Matrix）
...
如图36-4所示，数据等级、使用目的与处理动作三者组合可构成一个直观的风险矩阵，下表给出若干典型组合及其默认控制措施。
...
![图36-4：数据分级、用途和处理动作构成的风险矩阵图](../../images/part11/图36_4_数据分级用途和处理动作构成的风险矩阵图.svg)
*图36-4：数据分级、用途和处理动作构成的风险矩阵图*

### 36.2.6 责任链条构建（RACI Matrix）
...
当任意高风险项缺失时，流水线应阻断构建、阻断部署或阻断训练任务执行。图36-5给出了从数据接入到模型训练的合规门禁流程，展示各检查点如何在关键阶段拦截高风险变更。

![图36-5：从数据接入到模型训练的合规门禁流程图](../../images/part11/图36_5_从数据接入到模型训练的合规门禁流程图.svg)
*图36-5：从数据接入到模型训练的合规门禁流程图*

### 36.3.7 治理流水线：从文档要求走向系统执行
...
这一链路反映了一个关键事实：**合规不是单点动作，而是一条从策略生成、数据处理、告警响应到评估验证的流水线**。图36-3展示了隐私规格与策略的生成流程，是上述最小治理链路的起点环节。

![图36-3：隐私规格与策略生成流程图](../../images/part11/图36_3_P09隐私规格与策略生成流程图.svg)
*图36-3：隐私规格与策略生成流程图*

### 36.3.8 治理指标如何转化为工程语言
...
这类指标的意义，不在于绝对数值多大，而在于它们把合规治理从抽象理念转成了可检查、可复盘、可持续运行的系统行为。图36-2展示了 DPIA 与 RoPA 工程化审批流，即数据合规生命周期中从业务立项到下线的自动化拦截与审计流程。

![图36-2：DPIA与RoPA工程化审批流](../../images/part11/图36_2_DPIA与RoPA工程化审批流.svg)
*图36-2：数据合规生命周期 —— 从业务立项到下线的自动化拦截与审计流程*
...
上述运行期治理动作最终需要形成闭环，图36-6展示了审计日志、告警、事件响应与复盘之间的闭环关系。

![图36-6：审计日志、告警、事件响应与复盘闭环图](../../images/part11/图36_6_审计日志告警事件响应与复盘闭环图.svg)
*图36-6：审计日志、告警、事件响应与复盘闭环图*
...
图36-7给出了第三方 API 与大模型调用的边界网关结构，展示请求出域前如何统一完成检测、脱敏与阻断。

![图36-7：第三方 API / 大模型调用边界网关图](../../images/part11/图36_7_第三方API与大模型调用边界网关图.svg)
*图36-7：第三方 API / 大模型调用边界网关图*
...
### 36.5.1 治理工具箱：RoPA 声明配置（YAML 示例）
...
![图36-8：用户删除请求的全链路传播与清理示意图](../../images/part11/图36_8_用户删除请求的全链路传播与清理示意图.svg)
...
如果资源有限，最先上线的应是那些一旦缺失就极易引发事故的默认规则，例如：
...
本章论证了数据合规的成本会随项目生命周期推进而快速抬升，因而必须坚持合规左移与 Privacy by Design，将治理约束前置到需求、架构与研发阶段，而非上线前的审批附件。围绕这一主线，章节先建立三层数据分类分级体系，并从字段级、表级到场景级展开多维分级；再以“数据敏感度 × 处理动作强度 × 业务影响范围”构造风险评估矩阵，并通过 RACI 明确法务、业务、平台、算法、安全与审计的责任链。

在流程层面，本章把 RoPA 处理活动记录、DPIA 影响评估、最小必要原则、同意与用途绑定、审计留痕和 CI/CD 合规预检串成从立项到下线的门禁闭环，并针对医疗、金融、未成年人、外采数据、第三方 API、跨境传输与大模型 Prompt 等高风险场景给出专项控制。最后，章节以 RoPA 声明、分级策略、访问策略、DPIA 模板、审计日志与预检清单等配置化交付物，说明制度文本如何沉淀为可校验、可阻断、可复盘的系统行为，并给出从模板化到平台化的演进路径。
