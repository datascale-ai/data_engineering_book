...
如图36-2所示，数据等级、使用目的与处理动作三者组合可构成一个直观的风险矩阵，下表给出若干典型组合及其默认控制措施。
...
![图36-2：数据分级、用途和处理动作构成的风险矩阵图](../../images/part11/图36_4_数据分级用途和处理动作构成的风险矩阵图.svg)
*图36-2：数据分级、用途和处理动作构成的风险矩阵图。*
...
当任意高风险项缺失时，流水线应阻断构建、阻断部署或阻断训练任务执行。图36-3给出了从数据接入到模型训练的合规门禁流程，展示各检查点如何在关键阶段拦截高风险变更。

![图36-3：从数据接入到模型训练的合规门禁流程图](../../images/part11/图36_5_从数据接入到模型训练的合规门禁流程图.svg)
*图36-3：从数据接入到模型训练的合规门禁流程图。*
...
这一链路反映了一个关键事实：**合规不是单点动作，而是一条从策略生成、数据处理、告警响应到评估验证的流水线**。图36-4展示了隐私规格与策略的生成流程，是上述最小治理链路的起点环节。

![图36-4：隐私规格与策略生成流程图](../../images/part11/图36_3_P09隐私规格与策略生成流程图.svg)
*图36-4：隐私规格与策略生成流程图。*
...
这类指标的意义，不在于绝对数值多大，而在于它们把合规治理从抽象理念转成了可检查、可复盘、可持续运行的系统行为。图36-5展示了 DPIA 与 RoPA 工程化审批流，即数据合规生命周期中从业务立项到下线的自动化拦截与审计流程。

![图36-5：DPIA与RoPA工程化审批流](../../images/part11/图36_2_DPIA与RoPA工程化审批流.svg)
*图36-5：数据合规生命周期 —— 从业务立项到下线的自动化拦截与审计流程。*
