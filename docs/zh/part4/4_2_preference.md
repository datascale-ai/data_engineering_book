...
也正因为如此，偏好数据与奖励信号才会在模型生命周期中占据独立位置。偏好数据关注的不是知识补充，也不是标准答案本身，而是模型在多个可行回答之间的取舍。比如，哪些回答应被鼓励，哪些表达虽然顺耳但不合规，哪些说法在普通场景可以接受，但在高风险场景必须收紧。偏好学习处理的正是这种排序问题。
...
本章讨论偏好数据如何进入后训练流程。内容包括偏好对、奖励模型、过程奖励、多目标权衡、偏好来源、噪声治理，以及这些数据如何服务于 DPO、RM、RLAIF、PRM 等方法。这里要说明的重点是：偏好学习不只是再做一轮人工反馈，而是把组织对模型行为的要求，整理成可以标注、可以训练、可以评估的数据结构。
...
行为风格不是靠几句固定话术形成的，而是在大量具体回答中慢慢稳定下来的。用户觉得一个模型“专业”，通常不是因为它反复使用专业化措辞，而是因为它在边界场景中会说明依据、保留不确定性，并给出必要限制。用户觉得一个模型“有帮助”，也不只是因为它写得多，而是因为它能优先给出完成任务所需的信息。风格背后其实是模型长期形成的选择倾向。
...
偏好数据的作用，是把抽象的风格要求落到具体比较里。模型并不知道“更专业”“更稳健”这些口号是什么意思，它只能从样本中看到：在同一个输入下，哪种回答被选中，哪种回答被放弃。这样的比较积累到一定规模后，模型才会形成稳定的回答倾向。
...
模型风格能不能稳定，主要看偏好样本的方向是否一致。单条规则很难改变模型的默认行为，真正起作用的是大量样本反复强化同一种取舍。
...
偏好样本的问题不只在数量，更在方向。样本很多，但一批奖励谨慎，一批奖励强势，一批奖励简短，一批又奖励铺陈，模型最后学到的就会很摇摆。相反，样本量不一定特别大，只要关键取向在不同场景中反复出现，模型就更容易形成稳定习惯。偏好数据建设不能只看“标了多少”，还要看“同一种取舍有没有被持续强化”。
...
SFT 更适合学习参考答案，偏好对齐更适合处理候选答案之间的排序。很多问题并不是“对或错”，而是“哪个更合适”。比如，一个答案内容没错，但太冗长；一个答案信息很全，但边界不清；一个答案语气自然，却容易让用户误以为模型给出了确定承诺。这类差异很难只靠 SFT 表达，更适合放到偏好比较里处理。
...
过程奖励不是为了把每一步都写得更好看，而是为了告诉模型：哪些中间行为可以被重复，哪些行为即使最后碰巧成功，也不应该被鼓励。结果监督主要约束最终输出，过程监督则进一步约束行动路径。对于 Agent 和工具调用任务，这一点尤其重要。
...
多目标偏好的难点在于，不同维度经常相互拉扯。更安全可能带来更保守的表达，更简洁可能减少必要解释，更亲和也可能降低效率。数据设计不能把这些冲突全部压进一个总分里，而应该保留关键维度和胜负原因。这样模型版本变化时，团队才能判断它究竟是变好了，还是只是换了一个权衡点。
...
偏好数据不可能完全无噪声，治理的重点是把噪声分清楚。有些问题要靠流程解决，比如复标和仲裁；有些问题要靠建模处理，比如样本权重和来源分层；还有些问题只能通过版本隔离来降低影响。偏好标签不能被当作绝对真值，而应被看作带有来源和置信度的训练信号。
...
按这种方式建设偏好数据后，模型的风格、边界感和稳定性就不再完全依赖随机生成结果，而是可以被数据、训练和业务规范共同塑造。对需要上线大模型的团队来说，偏好数据的价值就在这里：它让“模型应该怎么表现”这件事，有了可以持续迭代的数据基础。
