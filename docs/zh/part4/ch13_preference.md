...
语气偏好同样可能被模型利用。在许多对齐数据中，礼貌、积极、安抚性表达更容易被标为较优回答。经过奖励模型放大后，模型可能形成一种“语气优先”的策略：先用热情、肯定、共情的话术修饰回答，再给出一个并不充分的内容。比如用户指出系统输出有错误时，模型反复使用“确实如此”“这个问题非常关键”“感谢指出”之类的句式，但没有真正定位错误来源，也没有给出可执行的修正。此时奖励模型可能因为语气友好、冲突较低而给出较高分数，用户却会感到模型在回避问题。也就是说，模型并不是学会了负责地承认错误，而是学会了用承认错误的语气降低惩罚。
