...
##### 11.5.1 安全性对齐与对抗性越狱（Jailbreak）的深度博弈
...
##### 11.5.2 数学推理：错误注入（RISE）与过程监督（PRM）
...
##### 11.5.4 智能体（Agent）与工具调用：轨迹剪枝与死锁打破
...
#### 11.6 标注噪音、大模型评判与自动化流水线

##### 11.6.1 量化人类的主观噪音与 Kappa 系数
...
#### 11.7 偏好数据的病理学分析与算法修正

##### 11.7.1 冗长偏差（Length Bias）与词汇量剥削
模型易学会“长回复即高分”的规则。**长度控制边距偏好优化（LMPO）**通过引入正则化项和基于 EMA 的动态长度缩放机制，惩罚仅靠字数取胜的样本，迫使模型提高信息密度。

##### 11.7.2 概率退化（Probability Degradation）与 NLL 锚定
...
### 11.8 工程实现（Engineering Implementation）
...
#### 11.8.2 LLM-as-a-Judge：让判别过程结构化
...
#### 11.8.5 Constitutional AI 的自动化闭环
...
### 11.9 性能与评估 (Performance & Evaluation)
...
## 11.9.4 避坑指南 (Pitfalls & Troubleshooting)
