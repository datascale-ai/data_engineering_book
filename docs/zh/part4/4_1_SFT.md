...
在大模型从“会生成”走向“可用、可控、可部署”的过程中，监督微调（Supervised Fine-Tuning，SFT）承担着一个极其关键但又经常被误解的角色。很多团队把SFT简单理解为“收集一些问答对，然后让模型学着回答”，但在真实工程中，这样的理解远远不够。对数据设计和标注团队来说，SFT 不只是整理问答样本。它更像是在样本中写清楚：模型要完成什么任务、遵守哪些边界、按什么格式输出，以及在具体场景中应该怎样表达。在模型上线前，SFT 是少数还能直接通过数据去调整模型行为的阶段。

本章面向负责SFT数据设计、模板编写、标注规范制定、质量验收与版本管理的团队成员，系统讨论SFT的目标、任务分层、指令体系、样本结构、质量评估与行业化落地问题。本章的重点是说明：高质量 SFT 数据不能只是零散问答的集合，而要围绕能力覆盖、任务模板、质量评估和版本管理来组织。它需要以能力覆盖为骨架，以任务模板为载体，以质量闭环为保障，以后续偏好数据和工具调用数据为延伸接口。
...
因此，本章会把 SFT 看作一个持续的数据工程过程，而不是训练前临时准备一批样本。这个工程的难点，不在于是否能够快速造出数十万条样本，而在于是否能让这些样本真正构成一套有组织、有层次、有反馈、有版本治理能力的指令体系。只有当SFT数据被当作“模型行为的显式设计文档”来建设时，它才能成为后续偏好对齐、工具调用、Agent轨迹学习和线上反馈迭代的可靠基础。
...
因此，设计团队在标注规范中应尽量避免“表面正确、实则信号稀弱”的样本。一个好的SFT样本，不仅要告诉模型最终答案是什么，还要通过任务结构和上下文条件，明确告诉模型“什么情况下该这样答，什么情况下不该这样答”。高质量 SFT 样本不只是给出答案，还要让模型看清楚：在什么条件下该这样答，哪些边界不能越过。
...
此外，很多高价值SFT任务根本不是单步问答。例如，多轮澄清需要模型先识别信息不足，再提出补充问题；工具辅助任务需要模型在回答前输出调用参数或行动计划；约束式输出要求模型以特定schema生成结构化结果；系统任务则要求模型在冲突指令下保持优先级一致。因此，SFT 数据设计更接近交互结构设计，不能只停留在收集答案上。
...
为了避免SFT目标在团队协作中变得空泛，实际项目中往往需要将其转写为更可执行的工程目标。很多团队在项目初期都会说，希望模型“更听话”“更稳定”“更适合业务”“别老犯那些明显错误”。这些说法本身没有问题，但如果不继续往下拆，就很难真正指导数据建设。因为标注团队不知道该优先写什么样的样本，审核团队不知道该按什么标准验收，算法团队也很难判断一次训练结果到底有没有向目标靠近。SFT 不能只靠“让模型更好”这类愿景推进，而要把目标拆到样本、模板、评分标准和回归集里。
...
约束式输出模板则面向结构化系统集成约束式输出并不只是要求模型输出 JSON，还要让模型学会字段约束、枚举范围、空值策略、缺失信息处理方式和字段间依赖关系。很多格式错误并不是模型不会括号，而是模板没有让模型学会“在不确定时如何合法地表达不确定”。
...
因此，模板设计应遵循“形态多样，但规则统一”的原则。不同模板的表面形式可以不同，但它们都应遵守统一的字段划分、统一的约束表达方式、统一的异常处理策略和统一的质量标准。模板的作用不只是让样本整齐，更重要的是保证监督信号能够复用、扩展和审查。
...
医疗response的差异也很明显。它通常不能太快进入“像诊断一样”的陈述，而应该先判断当前信息够不够、风险高不高、用户最需要的是科普、观察建议还是立刻就医的提醒。尤其在信息不足的时候，response更合适的写法往往不是直接猜一个方向，而是先把需要补充的症状问出来，或者先把高风险情况排除掉。好的医疗 response 不一定要写得最像医生，更重要的是在安全边界内帮助用户判断下一步该怎么做。
