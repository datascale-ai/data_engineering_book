...
**表 38-2：Agent 工具白名单与调用条件**

### 38.1.3 审批闸门的分层设计

![AI Agent决策工作流程图生成 (10)](C:\Users\admin\Desktop\AI Agent决策工作流程图生成 (10).png)

**图 38-1：Agent 权限分层审批流程**
...
![AI Agent决策工作流程图生成 (11)](C:\Users\admin\Desktop\AI Agent决策工作流程图生成 (11).png)
...
安全防护不是一次性配置，而是一个持续的攻防对抗过程。Agent 的安全团队应定期（至少每月一次）对 Agent 进行红队测试：

**测试维度：**

- **已知攻击向量回归测试**：使用已修复的注入案例进行回归测试，确保修复持续有效。
...
![AI Agent决策工作流程图生成 (13)](C:\Users\admin\Desktop\AI Agent决策工作流程图生成 (13).png)

**图 38-3：Agent 安全事件应急响应流程**
...
**从"人在环路中"到"人在环路旁"。** 当前的主流模式是 Human-in-the-Loop（人在环路中）——Agent 的每个关键决策都需要人工确认。未来的方向是 Human-on-the-Loop（人在环路旁）——Agent 自主执行大部分操作，人工从旁监督，仅在异常时介入。这种转变的前提是 Agent 的可靠性达到足够高的水平，以及回滚机制的完备性。
...
## 38.6 Checklist：Agent 安全与权限设计自查
...
- **Ch34**：Agent 架构与任务边界——本章的安全权限设计建立在六层架构之上。
- **Ch35**：采集清洗 Agent——本章的输入安全清洗直接影响其采集管道。
- **Ch36**：标注评测 Agent——本章的提示注入防御对合成数据生成尤为重要。
- **Ch37**：DataOps Agent——本章的回滚审批与 Ch37 的成本和操作管理衔接。


---

## 38.8 延伸阅读：Agent 安全的前沿挑战与最佳实践
...
Agent 安全不是一劳永逸的配置，而是一个需要持续演进的架构。以下是三个值得关注的前沿方向：
