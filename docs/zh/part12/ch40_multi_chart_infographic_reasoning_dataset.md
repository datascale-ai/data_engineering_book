...
- 视觉与上下文推理：综合图例符号、图标标记、页面注释、自然语言正文信息完成非数值类推理，关键信息不直接以数字形式展示在坐标轴中。如样本中 2018 马萨诸塞州高风险鲨鱼袭击物种答案（大白鲨），数据来源是图标图例标注（高风险袭击图标 + 鲨鱼物种备注），无法通过纯数字读取获得，必须结合视觉符号 + 文本备注综合推导。
...
- 子图 C（Tabular Chart）：美国近 10 年各州鲨鱼袭击汇总统计；关键数据：2018年发生在马萨诸塞州的高风险鲨鱼袭击品种为大白鲨；支撑子问题 4 数值来源。
