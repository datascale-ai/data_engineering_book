...
传统视觉问答（VQA）、图表问答（ChartQA）研究长期聚焦单图表独立解析任务，模型仅需依托单一柱状图、饼图或表格即可完成数值提取与简答，和现实场景中商业报告、行业研报、科普资讯里的复合型信息图应用范式存在巨大鸿沟。本章以多图表信息图推理数据集为落地案例，系统性拆解面向真实复合信息图的数据构建逻辑、任务定义、样本架构与评测体系。该数据集由 354 份真实场景复合信息图、1917 条前后逻辑关联的多阶子问题构成，覆盖 28 大应用领域、20 余种主流图表样式，核心任务拆解为跨图表数据聚合、多步骤数值运算、视觉与上下文语义推理三类。依托鲨鱼袭击统计多轮链式问答样本，具象化演示多问题链路跨子图调取数据、分步推导答案的推理逻辑；同时梳理数据集全流程构建流水线、多维度评测指标，归纳图例误读、跨图表数据混淆、前置计算错误逐级传导等典型模型失效模式（该数据集仅完成样本标注阶段，暂无配套落地算法与基线模型，是当前多模态图表推理领域稀缺的真实场景基准数据集，可为后续跨图表深度推理算法研发提供标准化评测底座），github 网址为 https://github.com/[xychen-zh/multi-chart-infographic-reasoning-dataset ](https://github.com/xychen-zh/multi-chart-infographic-reasoning-dataset) （目前在创建中）。

## 40.1 问题场景：单图表 VQA 的局限性与复合信息图推理的现实需求

### 40.1.1 传统单图表视觉问答任务边界约束

现有主流图表视觉问答数据集，如 ChartQA、FigureQA、PlotQA 等，在数据设计层面普遍遵循一图一问、单图闭环范式：单张样本图像只包含独立单一图表（单柱状图、单折线图、单饼图等），全部作答所需数据、图例、统计数值均被收拢在同一张可视化视图内，模型仅需要完成三项基础操作：定位图表坐标、读取标注数字、依托单图数据完成一步加减或分类判断。
...
- 视觉 + 上下文联合推理需求。信息图存在大量非数值类标注：物种备注、年份标注、图例符号（致命袭击 / 非致命袭击图标区分）、地域简写符号，部分关键信息不体现在坐标轴数字中，藏于图例、脚注、侧边说明文字内，需要模型结合视觉符号与自然语言上下文综合推理，即视觉与上下文推理任务。

### 40.1.3 现有基准数据集缺口与本数据集研发意义

当前全球公开的多模态图表推理基准数据集存在明显供给缺口：人工合成仿真图表数据集占比超 90%，基于网页、报刊、科普出版物抓取的原生真实复合信息图样本稀缺，多数数据集为了降低标注难度，人为拆分多子图信息图为多张独立图片，破坏原图的空间关联与上下文逻辑。在此背景下，多图表信息图推理数据集立足原生真实信息图抓取，保留原图多子图同屏布局、图例全局共用、分区备注穿插的原生结构，填补真实场景跨图表推理评测基准空白。
...
本章依托原始数据集文档数据，从样本体量、领域通用性、子图表类型、问答体系四个维度完成数据集全景梳理，精准还原 354 张原图、1917 条子问题的基础参数。
...
- 问答样本总量：全数据集配套 1917 条具备逻辑关联性的多阶子问题，单张复合信息图配套多条前后依赖的链式子问题，问题和答案一一人工标注核验；额外针对每张信息图追加 1 道无法依托图内现有信息作答的无效问题，用于测试模型拒答能力与鲁棒性，规避模型强行编造数据作答的过拟合缺陷。 从单图平均配比测算：单张信息图平均配套 5.41 条有效推理子问题 + 1 道不可作答测试题，问答密度贴合真实场景人类分步提问习惯。

### 40.2.2 全维度领域覆盖设计（28 大细分领域）
...
![ch40_domain](C:\Users\cxy\Desktop\ch40_domain.png)
...
![](C:\Users\cxy\Desktop\ch40_chart.png)
...
![](C:\Users\cxy\Desktop\ch40_question.png)

### 40.2.5 三大核心任务标准化定义

- 跨图表数据聚合：将分散在整图不同物理分区、不同类型子图表内的异构数据，按照问题限定的筛选条件完成归类、汇总、合并，打破子图物理分割边界，建立跨视图数据关联。例如鲨鱼案例中，各县历史袭击数（排名卡片子图）、各州十年累计袭击数（柱状统计表子图）、意外致死数据（象形对比子图）分属三个分区，聚合两类分区数据才能完成州际差值计算。该任务是本数据集区别于传统 ChartQA 最核心的标志。
- 多步骤串行运算：子问题按照前后依赖链式排布，前序问题输出答案作为后序问题的计算输入参数，需要依次执行查找、提取、加减、求差、平均、占比等串行算术操作，单条最终答案无法一步算出。鲨鱼样本 6 条子问题完整呈现该特征：问题 1 极值查找锁定县域→问题 2 依托县域所属州调取全州总量→问题 3 双州数值做差值运算，三步环环相扣。
...
本章依托数据集附带的鲨鱼袭击真实标注样例，从整图分层、子图分区、链式子问题、证据点位、分步推理路径五个维度具象化解析样本结构，落地诠释跨图表推理落地逻辑。

### 40.3.1 单张复合信息图的物理分层结构

![where-the-most-shark-attacks-occur-in-the-united-states(1)](C:\Users\cxy\Desktop\ch40_where-the-most-shark-attacks-occur-in-the-united-states(1).jpg)
...
| Q4   | 条件推理 | 2018 年马萨诸塞州致命鲨鱼袭击涉事物种？     | Presumed Great White | 子图 C                              | 无数值运算，依托符号与文本上下文推理          |
...
![ChatGPT Image 2026年6月5日 17_51_40](C:\Users\cxy\Desktop\ch40_pipeline.png)

### 4.1 第一步：真实复合信息图素材收集与初筛
...
- 图例误读失效：混淆致命袭击 / 非致命袭击图标含义，无法通过符号推导鲨鱼物种，Q4 作答错误
...
- 部分信息图原生瑕疵：少量网络原生信息图存在手写模糊标注、简写不规范问题，小幅提升识别难度，属于素材原生客观缺陷。
