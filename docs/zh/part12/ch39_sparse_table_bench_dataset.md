...
向后看，STB 直接连接第50章的 VLM 数据配方。第50章关注多模态训练数据如何组织图像、文本、坐标和指令信号，而 STB 正好提供了一个结构化视觉监督样例：输入是表格图像，输出同时包含 HTML 结构、单元格文本、bbox 和空单元格占位。它可以作为 VLM 数据配方中的文档表格切片，用来说明为什么通用图文对不足以训练稳定的表格结构能力。
