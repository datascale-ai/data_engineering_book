...
在第14篇项目中，STB 也可以连接 P03 和 P05。P03 的 LLaVA 多模态指令数据工厂需要把文档图像转成可训练的视觉指令样本，STB 可以提供“识别表格结构”“指出空单元格位置”“解释列位偏移错误”等指令来源。P05 的多模态 RAG 项目需要从 PDF、财报和科研报告中抽取可检索证据，STB 则能帮助把表格解析为可引用、可比对、可追踪的结构化证据。特别是在财报、医学论文和科研报告场景中，表格结构错误往往比单个 OCR 错字更难发现，也更容易影响最终回答。
