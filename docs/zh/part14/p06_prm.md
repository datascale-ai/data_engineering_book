...
本项目以“CoT 推理数据集与 PRM 训练”为核心案例，目标是构建包含正例、负例和修复路径的过程监督样本，用于训练或评估 PRM。读者完成本章后，应能够辨认该场景的关键数据对象、拆分工程链路、设置验收指标，并将案例方法迁移到相近的数据工程任务中。
...
在项目评审中，评审者很容易先问“做了多少条数据”。但对 PRM 来说，规模固然重要，结构更重要。
