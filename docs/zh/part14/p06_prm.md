...
CoT；PRM；过程监督；推理轨迹；奖励建模

## 项目目标与读者收获

本项目以“CoT 推理数据集与 PRM 训练”为核心案例，目标是构建包含正例、负例和修复路径的过程监督样本，用于训练或评估 PRM。CoT 轨迹设计参考 chain-of-thought prompting 的基本发现 (Wei et al. 2022)，过程监督与 step 级验证的验收口径参考 “Let's Verify Step by Step” (Lightman et al. 2023)。读者完成本章后，应能够辨认该场景的关键数据对象、拆分工程链路、设置验收指标，并将案例方法迁移到相近的数据工程任务中。

## 场景约束与数据边界

以可验证推理任务和结构化 step 标签为主，不覆盖所有开放域复杂推理。这些边界使案例能够被复现和审计；当数据规模、数据来源、权限范围或部署环境变化时，需要重新评估采样策略、质量阈值、运行成本和合规要求。

## 架构决策

本项目采用“种子任务、轨迹生成、step 切分、自动验证、标签设计和 PRM 数据封装”的架构路径。该决策优先保证输入输出契约、版本可追踪、异常可定位和结果可复核，而不是把全部逻辑压缩为一次性脚本运行。
...
验收指标包括step 标注一致性、验证通过率、正负样本比例、repair 覆盖、PRM 区分度和抽检错误率。若项目进入生产、课程或公开复现实验环境，还应记录版本号、依赖环境、随机种子、样本抽检结果和失败样本复盘记录。

| 验收维度 | 指标/证据 | 出版复核口径 |
| --- | --- | --- |
| 过程标签 | step 标注一致性、reward bucket 分布和验证通过率 | 抽检正例、负例和 repair 轨迹是否各有独立证据 |
| 训练接口 | PRM 样本字段完整率、训练切分记录和 manifest | 每批数据应能回到任务、轨迹、验证器和标签来源 |
| 噪声控制 | 负例污染率、修复轨迹歧义样本和人工复核结论 | 不把验证器未覆盖的样本直接视作高质量监督信号 |

*表 P06-1：过程监督数据出版验收表*

## 成本、风险与合规边界

成本主要来自多路径推理生成和验证；风险集中在错误链路被标成正例、验证器覆盖不足和过程标签噪声。涉及外部数据、个人信息、版权内容或第三方服务时，应保留来源说明、权限状态、脱敏策略、调用记录和人工复核记录。
...
第一类问题是**结果正确但过程不可用**。模型可能给出正确答案，但中间推理只是模板化铺陈、后验解释甚至随机拼接。这样的轨迹如果直接拿来做监督，模型学到的不是推理能力，而是一种围绕正确答案生成表面合理解释的模式。

第二类问题是**过程看起来合理，但关键步骤已经错了**。这类样本对结果监督来说只会被归为“答错”，但对过程监督来说，重要的是定位：到底是条件识别错了、公式替换错了、代码执行错了，还是修复路径本身有问题。
...
> 当团队未来需要把过程监督从数学和代码扩展到表格推理、工具调用、Agent 规划甚至复杂业务流程时，能被复用的不是某条思维链，而是这套“从任务到步骤监督”的工程方法。
...
![图 P06-1](../../images/part10/10_6_fig01_prm_factory_overview.png)
...
![图 P06-2](../../images/part10/10_6_fig02_step_validation_loop.png)
*图 P06-2：步骤级验证与训练闭环图*
...
但在真实工程里，PRM 训练的上限很大程度上取决于：**系统首先接触到什么任务、什么难度、什么类型的过程。**

### 5.1 为什么种子任务是必要的

如果没有明确的任务池，项目就容易退化成“随机生成一些 reasoning traces”，看起来很多，实际上分布混乱、难度不均，也无法解释为什么这些 step 值得监督。
...
种子任务只是一层“监督来源”，而不是最终监督单位。P06 的价值，在于它把种子任务扩展成多种轨迹，再把轨迹切成 step，最后才导出 PRM 数据。这条链路意味着：
...
`sampler.py` 在这一层负责从 GSM8K、MATH 和 MBPP 构造两类种子：数学任务抽取最终答案并拆成参考步骤，代码任务保留 `reference_code`、`test_setup_code` 与 `test_list`，最后统一落到 `seed_pool.jsonl` 与 `task_spec.json`。MATH 数据集的题目结构为多步推理和过程验证提供了典型来源 (Hendrycks et al. 2021)。这样可以在上游阶段就准备好后续 trace 生成和验证所需字段。
...
![图 P06-3](../../images/part10/10_6_fig03_task_sampling.png)
...
这会导致 PRM 容易退化成一个“偏好冗长且格式工整的轨迹”的打分器，而不是一个能识别过程质量的模型。
...
但 repair 轨迹也最容易引入噪声。因为如果修复逻辑本身不稳，模型学到的可能不是“如何纠错”，而是“如何把错误重新组织成表面合理的继续推理”。

### 7.4 当前项目的轨迹结构说明了什么

现有指标显示，项目生成了 108 条轨迹，其中三类轨迹完全对称：`positive=36`、`negative=36`、`repair=36`。这说明项目在轨迹结构上不是临时生成若干样本，而是明确把三类过程当作平行监督对象来设计。

![图 P06-4](../../images/part10/10_6_fig04_trace_types.png)
*图 P06-4：三类轨迹关系示意图*
...
![图 P06-5](../../images/part10/10_6_fig05_step_schema.png)
...
![图 P06-6](../../images/part10/10_6_fig06_validation_pipeline.png)
...
很多人在谈过程监督时，会用一些抽象概念，比如“高质量思维链”“可信步骤”“过程一致性”。但工程里能落地的，不是这些抽象词，而是**能进入字段、统计和训练的数据标签**。
...
![图 P06-7](../../images/part10/10_6_fig07_step_labels.png)
...
很多团队做过程评分时，容易把整个问题简化成“给轨迹打一个分”。这种做法当然可以快速验证，但也会很快碰到问题：分数太稀疏、边界太主观、训练接口不稳定。
...
P06 的流程中，这一步由 `src/prepare_prm_data.py` 负责。它的意义在于：项目最终建设的不是一堆零散中间文件，而是一套可训练的数据资产。
...
![图 P06-8](../../images/part10/10_6_fig08_training_interface.png)
...
在项目评审中，评审者容易先问“做了多少条数据”。但对 PRM 来说，规模固然重要，结构更重要。
...
第三，`144` 个 process-only signal steps 说明项目引入了 outcome 之外的新信号，而不是仅仅重排答案文本。
...
很多案例在写结果时，喜欢只报一个总量数字，比如“生成了 100 多条轨迹”。但更有价值的是那些能帮助理解系统行为的指标。
...
![图 P06-9](../../images/part10/10_6_fig09_validation_metrics.png)
...
项目章节最需要展示的是“工程闭环”，而不是最后模型分数。因为在很多真实团队里，最先出问题的不是 PRM 模型本身，而是：
...
一个没有检查层的 PRM 项目，容易在演示阶段看起来正常，在复现阶段却完全失控。把检查脚本写进章节主体，本质上是在强调：

> 过程监督项目的可信度，不只来自样本本身，也来自它是否能自证没有明显工程错误。
...
如果项目没有把二者区分开，PRM 很可能学到的是“如何惩罚风格奇怪的文本”，而不是“如何识别错误的推理步骤”。

### 16.2 为什么修复轨迹更容易产生歧义

repair 轨迹看起来很理想，因为它模拟了模型犯错后修复的过程。但难的是：
...
![图 P06-10](../../images/part10/10_6_fig10_noise_sources.png)
...
昂贵的，从来不是“多几个 step”，而是**在没有验证闭环之前盲目扩量**。
...
因为会被团队复用的案例，通常都不是“宣称已经做完”的案例，而是“把下一步应该怎么做说明白”的案例。
...
只要把“step”的定义从文本推理扩展到动作与状态，PRM 数据工厂的方法就有机会迁移到更复杂场景，相关交互模式可参考 ReAct 的推理-行动协同范式 (Yao et al. 2023)。若继续走向强化学习式推理飞轮，则应明确区分本章的过程监督数据构造与 DeepSeek-R1 所代表的大规模推理强化学习系统 (DeepSeek-AI 2025)。
...
这意味着它已经不再只是一个“带 CoT 的数据原型演示”，而更接近一个可供团队参考的过程监督工程案例。

可以概括为一句话：

> PRM 数据工厂要建设的，不是更长的答案，而是更可信的过程。
...
P06 已经把过程监督数据做成了训练接口，但进入训练时，还会遇到另一个现实问题：这些过程信号究竟应该怎么用。不同团队、不同阶段，对 PRM 数据的使用方式往往差异很大。
...
只有这些信息被稳定保留，团队才能评估“这版 PRM 比上一版好在哪里”。否则，即使训练结果变了，也很难判断变化来自哪一种过程信号调整。对于过程监督这种高度结构化的数据类型来说，版本语言本身就是工程能力的一部分。
...
许多过程监督项目的主要风险，不是当前存在噪声，而是每次迭代都重复遇到同类噪声。replay 集的作用，就是把这些问题从一次性经验，变成可持续复核的项目记忆。只要这份记忆在，团队就更容易回答一个关键问题：这版 P06 真的是更好了，还是只是换了一种方式重复过去的问题。

## 本章小结

本章以“CoT 推理数据集与 PRM 训练”为案例，展示了构建包含正例、负例和修复路径的过程监督样本，用于训练或评估 PRM的工程组织方式。案例的主要价值在于把任务定义、数据边界、架构决策、样本 schema、指标验收和复现资源放在同一条链路中，使项目不再只是操作步骤，而成为可复核的案例研究。

该案例的边界同样需要被清楚保留。以可验证推理任务和结构化 step 标签为主，不覆盖所有开放域复杂推理。在更大规模、更高风险或更强合规约束的场景中，应重新评估数据来源、权限状态、人工复核比例、运行成本和失败回滚方案。
