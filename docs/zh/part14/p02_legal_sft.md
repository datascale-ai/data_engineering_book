...
* 质量控制与偏好增强：通过质量保障（QA）、偏好对和风险边界样本稳定监督信号。
* 训练封装与上线验收：把处理后的数据资产整理成可训练、可验证、可交付的成品。

如果按工程顺序阅读，本章对应的是一条完整链路：

**原始法规 PDF -> 清洗切块 -> 任务设计 -> 指令合成 -> 偏好增强 -> 质量保障（QA）质检 -> 训练封装 -> 上线验收**
...
法律 SFT；垂直领域数据；指令体系；合规边界；标注质量

## 项目目标与读者收获

本项目以“法律领域专家 SFT 数据工厂”为核心案例，目标是把法规、判例和法律问答材料转化为可追踪的领域 SFT 训练资产。读者完成本章后，应能够辨认该场景的关键数据对象、拆分工程链路、设置验收指标，并将案例方法迁移到相近的数据工程任务中。

## 场景约束与数据边界

以公开或授权法律文本为输入，不覆盖真实法律咨询责任和生产级案件管理系统。这些边界使案例能够被复现和审计；当数据规模、数据来源、权限范围或部署环境变化时，需要重新评估采样策略、质量阈值、运行成本和合规要求。

## 架构决策

本项目采用“法律文档解析、任务模板、样本生成、质量复核、训练切分和风险标注”的架构路径。该决策优先保证输入输出契约、版本可追踪、异常可定位和结果可复核，而不是把全部逻辑压缩为一次性脚本运行。

## 样本 schema / 数据流

核心数据流可概括为：

Listing P02-1 给出了流程或路径示例，用于说明本节中的输入输出关系、结构约束或执行方式。
```text
法律文本/PDF -> 文档清洗 -> 领域任务 schema -> 指令样本 -> 合规与质量检查 -> SFT 数据集
```
...
验收指标包括任务覆盖度、条文引用准确性、样本格式合格率、人工抽检通过率、领域风险标签完整性和训练切分一致性。若项目进入生产、课程或公开复现实验环境，还应记录版本号、依赖环境、随机种子、样本抽检结果和失败样本复盘记录。

## 成本、风险与合规边界

成本主要来自文档解析、样本生成和专家复核；风险集中在法规时效、引用错配、未经授权文本和法律建议误用。涉及外部数据、个人信息、版权内容或第三方服务时，应保留来源说明、权限状态、脱敏策略、调用记录和人工复核记录。
...
复现材料应包括数据来源说明、最小样本、配置文件、运行命令、指标脚本、检查报告和产物目录。正文保留必要片段；完整 notebook、长脚本和大文件作为配套资源独立维护。法律 SFT 的训练封装仍可沿用通用文本到文本数据组织思想 (Raffel et al. 2020)，数据加载、批处理、实验记录和质量门可分别参考 Hugging Face Datasets (Hugging Face 2026)、Ray Data (Ray Project 2026)、MLflow (MLflow Authors 2026) 与 Great Expectations (Great Expectations Contributors 2026)。
...
> 当团队未来需要从法律迁移到税务、金融、医疗、客服等领域时，可以复用的不是某个 prompt，而是这套“从种子知识到监督数据”的工程方法。
...
**目标三：建立可审核、可拒收、可版本化的质量保障（QA）机制。**
法律数据如果只有生成没有审查，容易把错误样本批量放大。因此项目不仅有 SFT 样本，还建设偏好对、评审记录与风险拒答样本。
...
![图 P02-1](../../images/part14/p02/p02_01_legal_sft_factory_overview.svg)
...
因为很多团队第一次做行业 SFT 时，卡住的不是“不会写代码”，而是没有把生产过程拆成角色和环节，导致：
...
![图 P02-2](../../images/part14/p02/p02_02_roles_and_responsibilities.svg)
...
Listing P02-2 给出了 Python 实现片段，用于说明本节中的输入输出关系、结构约束或执行方式。
```python
...
Listing P02-3 给出了流程或路径示例，用于说明本节中的输入输出关系、结构约束或执行方式。
```text
……应当依法承担相应责任。 - 195 - 当事人……
```
...
如果简单写一个“删掉破折号数字破折号”的规则，容易误删正文中合法存在的编号或列表结构。因此，本项目通过更谨慎的正则约束页码的前后边界，只删除更像独立页码块的片段，而不轻易触碰正文编号。
...
Listing P02-4 给出了流程或路径示例，用于说明本节中的输入输出关系、结构约束或执行方式。
```text
法 律 规 定
合 同 关 系
```
...
![图 P02-3](../../images/part14/p02/p02_03_pdf_cleaning_pipeline.svg)
*图 P02-3：法律 PDF 智能清洗示意图*

![图 P02-4](../../images/part14/p02/p02_04_cleaning_examples.svg)
...
![图 P02-5](../../images/part14/p02/p02_05_seed_schema.svg)
...
而法律场景需要的，不只是复述规范文本，而是能在不同任务形式下稳定工作。因此，本项目把监督任务至少拆成三大类。
...
![图 P02-6](../../images/part14/p02/p02_06_task_taxonomy.svg)
*图 P02-6：法律任务体系分层图*
...
在本章示例产物统计中，三类主体任务各有 2577 条，用于说明任务结构相对均衡的验收口径。最终训练集规模为 7737 条，表示流水线可以从法规种子扩展出更完整的监督数据资产。与此同时，法律来源分布并不均衡：民法典相关样本 3882 条、刑法 1710 条、公司法 855 条，不同法域的覆盖仍存在明显集中。若这些数字用于正式交付，应随数据 manifest、生成脚本版本和抽样日志一并归档。
...
![图 P02-7](../../images/part14/p02/p02_07_task_vs_domain_distribution.svg)
...
如果完全依赖人工写作，项目会很快出现成本失控。在本章示例估算中，人工审核工时可按 193.28 小时、审核成本约 23193.6 元来展示成本核算方式；正式交付时应以审核记录、单价假设和抽样日志替换这些示例值。如果主体样本还全部靠人工撰写，整体投入会更高。相反，先利用教师模型自动扩张，再把人力集中到审核与难例上，是更现实的做法。

### 11.2 法律合成的约束

通用问答里，生成模型可以相对开放地写出各种自然语言回答；但法律场景里，自由发挥越大，风险就越高。因为模型容易：
...
![图 P02-8](../../images/part14/p02/p02_08_weighted_task_sampling.svg)
*图 P02-8：加权轮盘赌任务采样示意图*

---

## 12. 思维链（CoT）显性化：法律推理的表达约束

很多团队在做法律 SFT 时，容易把“专家感”理解成“回答写得很长、很正式”。但让模型更像法律助手的，往往不是篇幅，而是**推理过程是否可见、结论是否有层次、边界是否被表达出来**。
...
![图 P02-9](../../images/part14/p02/p02_09_cot_structure.svg)
...
![图 P02-10](../../images/part14/p02/p02_10_preference_and_review.svg)
...
因为模型学到的行为模式首先来自训练数据。如果训练集中大量存在：
...
![图 P02-11](../../images/part14/p02/p02_11_risk_refusal_flow.svg)
...
行业 SFT 最容易被低估的环节，就是 QA。很多时候，团队把大部分精力放在“怎么生成更多样本”，却忽略了决定上线质量的是“怎么挡住坏样本”。
...
![图 P02-12](../../images/part14/p02/p02_12_qa_loop.svg)
*图 P02-12：QA 审核闭环图*

![图 P02-13](../../images/part14/p02/p02_13_qa_decision_table.svg)
...
按本章示例口径，人工审核工时估算约为 193.28 小时，审核成本约为 23193.6 元。这个数字用于说明人审成本核算方法；若作为真实项目结果发布，必须补充审核记录、样本 ID、费率假设和复核人签核。如果继续扩量，而没有更合理的分层审核与供应商协同机制，成本会迅速失控。
...
![图 P02-14](../../images/part14/p02/p02_14_human_in_the_loop.svg)
...
![图 P02-15](../../images/part14/p02/p02_15_training_artifacts.svg)
*图 P02-15：训练封装与交付接口图*
...
从示例产物看，P02 可以形成比较完整的法律领域监督资产，并通过下游轻量验证说明监督信号的检查方式。若这些数字被用作真实项目结果，需与 `training_manifest.json`、生成脚本版本、随机种子、样本 ID 和评分记录逐项对应。
...
这说明示例流水线可以从原始法规种子扩展出结构较完整的监督资产，而不是停留在少量手工样本或一次性脚本演示上。
...
![图 P02-16](../../images/part14/p02/p02_16_metrics_dashboard.svg)
...
在前面的版本中，P02 已经能说明“数据工厂验证了”。但仅仅证明流程存在还不够，更重要的是回答下面这个问题：
...
示例验证在固定随机种子 `seed = 20260409` 下，随机抽样 50 条配对样本，对 `chosen` 与 `rejected` 两类数据做轻量质量验证。这里的目标并不是追求完整的模型基准测试，而是以一种成本可控、可复现的方式，快速回答偏好和 QA 是否真的拉开了质量差距。正式出版或交付时，应保留抽样脚本、样本 ID 列表和评分结果。
...
在这次示例 50 条抽样中，项目使用了几类非常有代表性的指标：
...
这组数字来自示例轻量抽样，正式发布时应附抽样脚本和评分明细；在教学口径下，它说明：
...
第二，说明“法律依据显式化”这条设计是有效的。`chosen` 样本在法条引用覆盖率上达到 100%，而 `rejected` 为 0%，说明当前模板与质检机制已经能显著推动模型输出更像法律说明，而不是泛泛而谈。
...
![图 P02-17](../../images/part14/p02/p02_17_eval_sampling_protocol.svg)
...
193.28 小时的人审成本在小规模演示项目里已经不算低。否则容易误以为“多生成一点法律数据只是多花一点 API 钱”。实际上，贵的往往是后面的审核与返工。
...
![图 P02-18](../../images/part14/p02/p02_18_version_timeline.svg)
...
很多团队在做大模型数据项目时，最先想到的是模型 API 成本。但在法律 SFT 里，昂贵的部分通常不是生成，而是：
...
因为项目部分不只是展示“方法可行”，还应该说明投入产出关系。如果一个方案理论上很好，但现实里无法承受其人力成本，那么它就不是可落地的方法。
...
![图 P02-19](../../images/part14/p02/p02_19_validation_chain.svg)
...
虽然合成是成本可控的必要手段，但合成比例过高会带来模板腔和教师偏置问题。模型可能学会“像模板那样回答”，却不一定掌握多样的用户表达。

### 25.3 风险拒答样本仍然偏少

当前已经建立了风险拒答机制，但样本规模仍小。对于上线场景，这还远远不够，特别是在个案化建议、敏感争议、证据不足判断等场景下，需要更丰富的拒答与保留边界样本。
...
能迁移的不是某个具体 prompt，而是这条方法链：

> 找到权威种子 -> 做结构化切块 -> 设计任务体系 -> 受控合成扩张 -> 建立 QA 与偏好 -> 单独建模风险边界 -> 训练封装与一致性验证。

![图 P02-20](../../images/part14/p02/p02_20_cross_domain_transfer.svg)
*图 P02-20：行业迁移方法链图*
...
回看整个项目，会发现它解决的问题，并不是“怎样让模型多生成几条法律问答”，而是：
...
本章以“法律领域专家 SFT 数据工厂”为案例，展示了把法规、判例和法律问答材料转化为可追踪的领域 SFT 训练资产的工程组织方式。案例的主要价值在于把任务定义、数据边界、架构决策、样本 schema、指标验收和复现资源放在同一条链路中，使项目不再只是操作步骤，而成为可复核的案例研究。

该案例的边界同样需要被清楚保留。以公开或授权法律文本为输入，不覆盖真实法律咨询责任和生产级案件管理系统。在更大规模、更高风险或更强合规约束的场景中，应重新评估数据来源、权限状态、人工复核比例、运行成本和失败回滚方案。
