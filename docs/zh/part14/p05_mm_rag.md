...
本项目采用“页面渲染、视觉索引、多页召回、证据组织、多模态回答和引用评测”的架构路径。该决策优先保证输入输出契约、版本可追踪、异常可定位和结果可复核，而不是把全部逻辑压缩为一次性脚本运行。
...
验收指标包括检索命中率、引用准确率、答案关键词准确率、延迟、页面处理成本和干扰页抑制效果。若项目进入生产、课程或公开复现实验环境，还应记录版本号、依赖环境、随机种子、样本抽检结果和失败样本复盘记录。

*表 P05-1：多模态 RAG 出版验收表*

| 验收维度 | 指标/证据 | 出版复核口径 |
| --- | --- | --- |
| 证据检索 | Top-K 命中率、目录页误召回率和跨页召回覆盖 | 每个答案应能回到页面图像、页码和检索分数 |
| 回答质量 | 引用准确率、数值一致性、趋势判断准确率和拒答覆盖 | 财务数值、图表趋势和结论必须能被人工复核 |
