...
本项目的出版重点不是证明系统“能够回答问题”，而是证明回答能够回到可核查的证据对象：财报页码、页面截图、表格区域、脚注说明、图表或正文段落。对财务、审计和投研场景而言，答案如果不能定位到证据，即使语言流畅，也不能视为稳定交付。
...
多模态 RAG；财报问答；视觉检索；证据引用；成本评估

## 项目目标与读者收获

本项目以“多模态 RAG 企业财报助手”为核心案例，目标是把复杂财报 PDF 组织成可检索、可引用、可评测的多模态问答系统。读者完成本章后，应能够辨认该场景的关键数据对象、拆分工程链路、设置验收指标，并将案例方法迁移到相近的数据工程任务中。

## 场景约束与数据边界

以页面级视觉检索和单文档/少量文档财报场景为主，不覆盖生产级全库财务分析。这些边界使案例能够被复现和审计；当数据规模、数据来源、权限范围或部署环境变化时，需要重新评估采样策略、质量阈值、运行成本和合规要求。

## 架构决策

本项目采用“页面渲染、视觉索引、多页召回、证据组织、多模态回答和引用评测”的架构路径。视觉索引可参考 CLIP 的图文表示学习思想 (Radford et al. 2021)，复杂文档问答和图表问答的评测口径可分别参考 DocVQA (Mathew et al. 2021) 与 ChartQA (Masry et al. 2022)。该决策优先保证输入输出契约、版本可追踪、异常可定位和结果可复核，而不是把全部逻辑压缩为一次性脚本运行。
...
验收指标包括检索命中率、引用准确率、答案关键词准确率、延迟、页面处理成本、干扰页抑制效果和证据回链完整性。若项目进入生产、课程或公开复现实验环境，还应记录版本号、依赖环境、随机种子、样本抽检结果和失败样本复盘记录。

| 验收维度 | 指标/证据 | 出版复核口径 |
| --- | --- | --- |
| 证据检索 | Top-K 命中率、目录页误召回率、跨页召回覆盖和证据页稳定性 | 每个答案应能回到页面图像、页码、检索分数和命中原因 |
| 证据定位 | 表格区域、脚注说明、图表页、正文页和截图证据的回链记录 | 数值、趋势和解释性结论必须能指向至少一个可复核证据对象 |
| 回答质量 | 引用准确率、数值一致性、趋势判断准确率和拒答覆盖 | 财务数值、图表趋势、脚注限制和结论必须能被人工复核 |
| 成本边界 | 页面渲染成本、视觉模型调用次数和平均延迟 | 交付时说明小样本演示与生产全库规模的差异 |

*表 P05-1：多模态 RAG 出版验收表*

## 成本、风险与合规边界

成本主要来自页面渲染、多模态索引和视觉模型调用；风险集中在财务数字误读、图表幻觉、目录页误召回和版权边界。涉及外部数据、个人信息、版权内容或第三方服务时，应保留来源说明、权限状态、脱敏策略、调用记录和人工复核记录。
...
复现材料应包括数据来源说明、最小样本、配置文件、运行命令、指标脚本、检查报告和产物目录。正文保留必要片段；完整 notebook、长脚本和大文件作为配套资源独立维护。若将本项目扩展为更通用的视觉指令或图文问答数据，可参考 LLaVA 的视觉指令调优范式 (Liu et al. 2023) 以及 COCO 的通用图像标注组织方式 (Lin et al. 2014)。
...
**目标三：让回答具备可解释性与可定位性。**  
系统输出不能只是一个“看起来合理”的总结，而应能够指向具体页面、页码、页面截图、表格区域、脚注说明或证据组合，便于复核与调试。
...
![图 P05-1](../../images/part10/10_5_fig01_overall_architecture.png)
...
Vision-first 的核心思想是：**先保留页面作为图像整体的表达能力，再让检索模型去学习“哪一页看起来接近答案语义所在页”**。
...
![图 P05-2](../../images/part10/10_5_fig02_vision_vs_ocr.png)
...
![图 P05-3](../../images/part10/10_5_fig03_page_assets.png)
...
`store_collection_with_index=True` 这一点很关键。因为多模态 RAG 的生成阶段并不是从文本库中取字符串，而是要把命中的**原图或页面截图**重新输入 VLM。没有这个关联，检索与生成就断开了。
...
![图 P05-4](../../images/part10/10_5_fig04_indexing_pipeline.png)
...
财报问答里，一个非常典型的问题是：**用户问的问题是跨页的，但系统只想返回一页**。
...
对纯文本 embedding 来说，这类页面非常“接近答案语义”。但对用户来说，目录页往往是信息密度最低的一页，因为它只提示“答案可能在哪个章节”，却不提供实际数据。
...
很多原型演示会把“检索到正确页面”当作默认前提。但真实系统要考虑的是：
...
![图 P05-5](../../images/part10/10_5_fig05_topk_filtering.png)
...
![图 P05-6](../../images/part10/10_5_fig06_multi_image_prompting.png)
...
content_payload = [{
    "type": "text",
    "text": (
        f"你是专业的 CFO 助手。现提供 {len(results)} 张财报截图。"
...
严格来说，这一步在很多原型演示中都被省略了。但在工程里，它非常重要。建议至少返回：
...
一次示例运行记录可以写为：系统针对“经营结果如何？”这一问题，召回第 49、91、130、8 页，并综合这些页面生成关于研发投入、无形资产和社会责任等方面的分析。正式交付时，应同时保存检索日志、页码证据、模型原始输出和版本信息。
...
在工程实践里，仅有“理论上可行”的章节不足以支撑复现。一旦放入真实日志，读者就能看到：
...
复杂文档问答中常见的风险是：回答听起来很专业，但证据未必正确。
...
* 证据回链完整率：回答中的关键结论是否能回到页码、截图、表格、脚注或图表证据；
* 数值正确率：关键数字是否抄对、比对对；
* 趋势理解正确率：图表趋势是否被正确解读；
* 干扰鲁棒性：加入目录页后性能下降多少；
...
![图 P05-7](../../images/part10/10_5_fig07_eval_framework.png)
...
复杂文档项目的核心风险，是“效果演示能跑，但缺少长期使用所需的成本边界”。成本分析写清楚，读者才能判断这套方案适合：
...
![图 P05-8](../../images/part10/10_5_fig08_optimization_roadmap.png)
...
![图 P05-9](../../images/part10/10_5_fig09_hybrid_rag.png)
...
* 有从页面资产到视觉索引的链路；
* 有多页召回与抗目录干扰设计；
* 有示例运行记录和可补充为正式日志的证据字段；
* 有基础评测与验证结果；
* 也有成本与后续优化方向。 

由此可以看出，它已经不再只是一次性多模态原型演示，而更接近一个可供团队参考的复杂文档 RAG 工程案例。
...
* 回答应附带页码、截图证据、表格或脚注说明，便于人工复核。

这种门禁的本质，是把“答得像专家”转成“答得经得起核查”。在财报场景里，后者比前者重要得多。

### 四、运营门禁：系统要能被长期维护

上线前还有一类经常被忽视的门禁，就是运营门禁。也就是说，即使系统指标表现可接受，如果没有稳定的索引重建、日志管理、评测回归和异常处理机制，也很难长期维护。
...
只有把运营门禁也纳入上线条件，多模态 RAG 才能从“一次效果展示”变成“一个有人敢持续使用的系统”。
...
对这类系统来说，最有效的协作节奏通常不是围绕“今天又新增了什么功能”，而是围绕“本周最值得修复的失败案例是什么”。一个实用的迭代节奏可以是：
...
* 核心结论，直接回答用户当前问题；
* 证据说明，指出主要依据的页码、页面类型、截图证据、关键表格/图表或脚注；
...
本章以“多模态 RAG 企业财报助手”为案例，展示了把复杂财报 PDF 组织成可检索、可引用、可评测的多模态问答系统的工程组织方式。案例的主要价值在于把任务定义、数据边界、架构决策、样本 schema、指标验收和复现资源放在同一条链路中，使项目不再只是操作步骤，而成为可复核的案例研究。

该案例的边界同样需要被清楚保留。以页面级视觉检索和单文档/少量文档财报场景为主，不覆盖生产级全库财务分析。在更大规模、更高风险或更强合规约束的场景中，应重新评估数据来源、权限状态、人工复核比例、运行成本和失败回滚方案。
