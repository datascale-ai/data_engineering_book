...
合成教材；程序化验证；数学数据；代码数据；课程结构

## 项目目标与读者收获

本项目以“合成数学与代码教材工厂”为核心案例，目标是用可验证的题目进化和解题链路生成数学与代码教材型数据。读者完成本章后，应能够辨认该场景的关键数据对象、拆分工程链路、设置验收指标，并将案例方法迁移到相近的数据工程任务中。

## 场景约束与数据边界

聚焦数学、代码和程序化推理样本，不替代完整课程教材编写与专家审稿流程。这些边界使案例能够被复现和审计；当数据规模、数据来源、权限范围或部署环境变化时，需要重新评估采样策略、质量阈值、运行成本和合规要求。

## 架构决策

本项目采用“种子题、Evol-Instruct、程序化推理、沙箱验证、难度分层和教材卷册组织”的架构路径。该决策优先保证输入输出契约、版本可追踪、异常可定位和结果可复核，而不是把全部逻辑压缩为一次性脚本运行。
...
验收指标包括验证通过率、难度分布、主题覆盖、解法一致性、错误样本占比和卷册结构完整性。若项目进入生产、课程或公开复现实验环境，还应记录版本号、依赖环境、随机种子、样本抽检结果和失败样本复盘记录。

| 验收维度 | 指标/证据 | 出版复核口径 |
| --- | --- | --- |
| 数据闭环 | 种子题、进化题、PoT 解法、沙箱日志和教材卷册均可追溯 | 抽检样本必须能回到原始种子与验证记录 |
| 质量指标 | 验证通过率、难度分布、主题覆盖和错误样本占比 | 指标口径写入报告，不以单次 100% 通过率替代复核 |
| 专家审稿 | 教学解释、章节顺序、难度梯度、术语一致性和高风险样本复核记录 | 专家复核意见应区分“答案正确”“解释可教”“适合入册”三个层次 |
| 代码隔离 | 可执行验证脚本、展示用不可执行代码、沙箱日志和依赖清单 | 公开正文中的代码片段不得默认作为生产脚本执行，配套脚本需在隔离环境中验证 |
| 决策边界 | 保留代码安全、版权混入和专家审稿边界 | 公开交付前确认题源授权、不可执行代码隔离和失败样本返工记录 |

*表 P04-1：合成教材工厂出版验收表*

## 成本、风险与合规边界

成本主要来自生成调用、沙箱执行和错误样本复核；风险集中在伪正确答案、代码安全、题目重复和版权混入。涉及外部数据、个人信息、版权内容或第三方服务时，应保留来源说明、权限状态、脱敏策略、调用记录和人工复核记录。
...
复现材料应包括数据来源说明、最小样本、配置文件、运行命令、指标脚本、检查报告和产物目录。正文保留必要片段；完整 notebook、长脚本和大文件作为配套资源独立维护。教材样本最终仍需要被组织成稳定的文本到文本或指令训练格式 (Raffel et al. 2020)，数据集管理、并行生成、实验追踪和质量检查可分别参考 Hugging Face Datasets (Hugging Face 2026)、Ray Data (Ray Project 2026)、MLflow (MLflow Authors 2026) 与 Great Expectations (Great Expectations Contributors 2026)。

## 1. 项目背景：合成数学与代码教材工厂的必要性

![图 P04-1](../../images/part10/10_4_fig01_project_positioning.png)
...
第一，**表面正确不等于可验证正确**。
模型很擅长写出“表面上具有推理形式”的答案，但这些推理文本里经常混入隐式跳步、数值替换错误、变量定义不一致，或者前文说 12 后文却按 15 继续算。从表面上看，它们很像正确解法；对于训练系统来说，这类样本会把错误逻辑呈现为高质量解释。
...
![图 P04-2](../../images/part10/10_4_fig02_goals_and_scope.png)
...
项目规模较小，但流程完整。它的价值不在于样本量特别大，而在于把从生成到交付的工程链条验证。因此更适合作为实践案例和小规模验证方案。
...
![图 P04-3](../../images/part10/10_4_fig03_pipeline_overview.png)
*图 P04-3：P04 整体架构总览图*
...
这一层不是简单随机采样，而是在为后续教材组织做准备。因为如果上游种子分布失衡，后续再怎么组织，整本教材都会偏科。
...
![图 P04-4](../../images/part10/10_4_fig04_roles_and_responsibilities.png)
*图 P04-4：教材工厂职责协同图*
...
![图 P04-5](../../images/part10/10_4_fig05_seed_to_plan.png)
...
![图 P04-6](../../images/part10/10_4_fig06_evol_path.png)
...
![图 P04-7](../../images/part10/10_4_fig07_cot_vs_pot.png)
...
**第二，可验证。**  
代码能验证，说明至少求解过程的结构是自洽的；结果正确，则进一步提升了监督可信度。
...
![图 P04-8](../../images/part10/10_4_fig08_generation_chain.png)
*图 P04-8：生成链路细化图*
...
![图 P04-9](../../images/part10/10_4_fig09_sandbox_validation.png)
*图 P04-9：沙箱验证执行路径图*
...
教材数据的关键风险是：内容看上去像老师讲义，但答案依据并不稳固。沙箱执行让项目能够在样本进入教材卷册之前，就先排除掉一批不可信内容。

> 没有验证的教材，更像排版完整的文本；有验证的教材，才更接近可训练资产。

---

## 11. 教材打包：课程化资产组织

![图 P04-10](../../images/part10/10_4_fig10_packaging_outputs.png)
*图 P04-10：教材打包产物关系图*
...
很多工程项目一谈教师指南就觉得像教学附属材料。但在教材型数据工厂里，教师指南其实有很强的工程意义：它是面向评审、标注、人工 QA 和训练运营人员的解释层。它能说明每本卷册教什么、如何使用、覆盖什么学习阶段，也能作为版本更新时的沟通接口。

---

## 12. 训练封装：教材数据进入训练系统

![图 P04-11](../../images/part10/10_4_fig11_training_interface.png)
*图 P04-11：训练封装接口图*
...
教学化示例运行可按以下量级估算：生成一条复杂样本（Evol + Code）可能需要 30–60 秒，1000 条高质量验证数据的 API 成本可能低于 1 美元。该数值仅用于说明成本分析方法，正式项目应以实际运行日志和 API 账单替换。
...
在本章示例估算下，API 成本不是主要瓶颈；正式项目仍需以实际账单、模型版本和重试率核算。更值得投入的，其实是：
...
如果没有这套标尺，团队很容易出现两种误判。第一种误判是把“代码能跑”当成“教材可用”。但真实情况是，代码即使验证，也可能存在题意不清、解释跳步、难度错位、章节重复等问题。第二种误判是把“章节写得像教材”当成“训练可用”。但训练侧真正关心的是 schema 一致性、切分策略、字段稳定性和分布平衡。两种误判叠加，最终会让项目停留在表面完成、实际上难以复用的状态。
...
本章以“合成数学与代码教材工厂”为案例，展示了用可验证的题目进化和解题链路生成数学与代码教材型数据的工程组织方式。案例的主要价值在于把任务定义、数据边界、架构决策、样本 schema、指标验收和复现资源放在同一条链路中，使项目不再只是操作步骤，而成为可复核的案例研究。

该案例的边界同样需要被清楚保留。聚焦数学、代码和程序化推理样本，不替代完整课程教材编写与专家审稿流程。在更大规模、更高风险或更强合规约束的场景中，应重新评估数据来源、权限状态、人工复核比例、运行成本和失败回滚方案。
