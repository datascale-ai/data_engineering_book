...
Agent Tool-Use；工具调用；执行轨迹；恢复建模；安全阻断
...
以模拟环境和可控工具集为主，不代表开放互联网或真实企业权限环境。这些边界使案例能够被复现和审计；当数据规模、数据来源、权限范围或部署环境变化时，需要重新评估采样策略、质量阈值、运行成本和合规要求。

## 架构决策

本项目采用“工具规范、任务模板、模拟执行、轨迹采样、失败恢复和格式验收”的架构路径。该决策优先保证输入输出契约、版本可追踪、异常可定位和结果可复核，而不是把全部逻辑压缩为一次性脚本运行。
...
> 当团队未来需要从简单单工具问答迁移到复杂多工具 Agent、企业 Copilot、工作流助手和具身任务代理时，可以复用的不是某个函数调用 prompt，而是这套“从工具规范到监督轨迹”的工程方法。

![图 P07-1](../../images/part14/p07/p07_01_agent_tooluse_factory_overview.svg)
...
![图 P07-2](../../images/part14/p07/p07_02_three_layer_architecture.svg)
...
这一层负责定义工具边界、任务类型、恢复规则和安全约束。这里要先回答的是：什么叫“合理的 Agent 行为”，而不是只关心某条轨迹能不能验证。
...
因为很多团队第一次做 Agent 数据时，卡住的不是“不会写函数调用”，而是没有把这些关键面前置写清，导致：
...
![图 P07-3](../../images/part14/p07/p07_03_roles_and_responsibilities.svg)
...
schema 不是为了好看，而是为了让“工具定义—轨迹生成—环境执行—训练封装—评估检查”这几层之间能对齐。没有这层对齐，Agent 项目容易变成一堆互相孤立的脚本。

![图 P07-4](../../images/part14/p07/p07_04_tool_schema_structure.svg)
...
![图 P07-5](../../images/part14/p07/p07_05_task_specs_and_templates.svg)
...
![图 P07-6](../../images/part14/p07/p07_06_trajectory_taxonomy.svg)
...
![图 P07-7](../../images/part14/p07/p07_07_simulated_env_loop.svg)
*图 P07-7：模拟工具环境执行闭环图*
...
![图 P07-8](../../images/part14/p07/p07_08_pipeline_steps.svg)
*图 P07-8：P07 六步流水线图*
...
![图 P07-9](../../images/part14/p07/p07_09_recovery_flow.svg)
*图 P07-9：参数修复与重试流程图*
...
很多人第一次做 Agent 时，会把记忆简单理解为“把前文多拼一点”。但工程上 memory 行为远不止如此。
...
![图 P07-10](../../images/part14/p07/p07_10_memory_trajectory.svg)
...
因为安全边界如果只在推理侧用规则补，容易出现“模型想做，规则在拦”的对抗状态。更好的方式是让模型在训练时就学会哪些事情不该做。

![图 P07-11](../../images/part14/p07/p07_11_unsafe_block.svg)
...
![图 P07-12](../../images/part14/p07/p07_12_dataset_repacking.svg)
...
做 Agent 项目时，很多团队最容易盯住一个数字：工具调用成功率。这个指标当然重要，但如果只看它，容易误判整个项目。
...
如果只报数字，78.57% 容易被误读为“偏低”的结果。但一旦放回 Agent 场景中，它反而说明项目没有美化数据，而是在如实保留并利用失败恢复行为。
...
![图 P07-13](../../images/part14/p07/p07_13_eval_and_checks.svg)
*图 P07-13：评估与检查双闭环图*
...
因为一个方法样板价值，不在于假装自己已经解决了一切，而在于让后来者知道：下一步最值得投入的地方在哪里。
...
未来可以引入更丰富的越权调用、提示注入、敏感信息泄露、数据污染和策略绕过场景，让安全边界接近上线前要求。

### 19.5 扩展评估维度

除了当前指标外，还可以增加更细粒度的工具选择准确率、参数正确率、重试效率、最终回答质量和多轮一致性指标。

![图 P07-14](../../images/part14/p07/p07_14_roadmap.svg)
...
在许多入门材料中，大模型工程仍停留在“让模型回答更像样”这一步。但 Agent 场景提出了一个更高的要求：模型不仅要会说，还要会做；不仅要会做，还要能在做错时修回来；不仅要能修回来，还要知道什么时候根本不该做。
...
因为很多团队会低估 Agent 数据难度，觉得多加几个函数调用样本就够了。P07 恰好说明：困难的不是“能不能调”，而是“调错了怎么办、该停时能不能停、跨轮时能不能记住”。
...
这份交付物列表说明，P07 最终沉淀下来的不是“验证一次实验”，而是一套从工具定义到训练接口再到评估报告的完整资产。

---

## 24. 结语：Agent 数据工厂要训练的，不只是调用动作，而是行为能力
...
> 如果我们想让模型学会“做事”，那就必须把行为本身变成数据工程对象。
...
Agent Tool-Use 项目在落地前，最容易出问题的地方并不是“模型会不会调工具”，而是“团队有没有把场景库和门禁条件建完整”。如果只有少量演示任务，系统看起来很聪明；一旦进入真实环境，工具空间、异常类型和安全要求同时放大，问题就会迅速暴露。
...
这样建设出来的场景库，才能代表系统最脆弱的边界，而不是只代表系统最擅长展示的部分。
...
只有把复盘粒度落在行为链上，团队才有机会把问题转回训练资产，而不是停留在“这次又没做对”的模糊描述里。
...
只要这几件事开始形成固定节奏，Agent Tool-Use 数据工厂就不再只是一个项目章节，而会逐步演化成行为数据基础设施。

## 本章小结

本章以“Agent Tool-Use 数据工厂”为案例，展示了把工具 schema、任务环境和交互轨迹组织成 Agent 工具使用训练数据的工程组织方式。案例的主要价值在于把任务定义、数据边界、架构决策、样本 schema、指标验收和复现资源放在同一条链路中，使项目不再只是操作步骤，而成为可复核的案例研究。

该案例的边界同样需要被清楚保留。以模拟环境和可控工具集为主，不代表开放互联网或真实企业权限环境。在更大规模、更高风险或更强合规约束的场景中，应重新评估数据来源、权限状态、人工复核比例、运行成本和失败回滚方案。
