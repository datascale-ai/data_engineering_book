...
第十三篇已经覆盖了数据流水线、SFT、多模态、RAG、Agent Tool-Use、DataOps、隐私保护和数据飞轮等项目。DataAgent 项目适合作为新增项目十一，原因在于它不是单点算法项目，而是一个把 Agent、语义层、NL2SQL、工具编排和服务接口连接起来的应用级数据工程实战。
...
SCENARIO:
  chat:
    ...
    instructions: |
      你是企业数据分析助手。
      当用户问题需要查询数据库时，必须调用 nl2sql_sub_agent_tool。
      调用工具时，query 必须写清楚业务目标、指标口径、过滤条件、分组粒度、排序规则和输出字段。
      每次调用必须提供 sql_filename 和 csv_filename。
      回答时说明 SQL 文件路径、CSV 结果路径、核心查询结果和可能的数据口径限制。
      不要编造未出现在查询结果中的数字。
    output_format: "业务解释 + SQL 文件路径 + CSV 文件路径 + 风险说明"

TOOLS:
  local_functions:
    - module: "dataagent.actions.tools.local_tool.tools"
      function: "nl2sql_sub_agent_tool"
      description: "将自然语言查询交给 NL2SQL 子 Agent，并保存 SQL 与 CSV 结果。"
      config:
