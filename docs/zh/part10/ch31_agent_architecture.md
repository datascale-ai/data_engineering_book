...
数据工程 Agent 的核心难点不在于让模型多调用几个工具，而在于把工具调用约束在可配置、可验证、可回放、可交付的系统边界内。本章先以单体 Agent 越界改写时间戳字段、污染下游管道的事故为起点，归纳出 Planner 与 Executor 职责合并、Human Gate 超时自动批准、Lineage 记录缺失三类系统性问题，由此提出 Planner、Tool Executor、Verifier、Human Gate、Memory 与 Lineage 六层架构，逐层界定不可越界的职责、跨层结构化通信协议与故障分级降级策略，使任一组件失效时系统默认转入更保守的人工模式。
