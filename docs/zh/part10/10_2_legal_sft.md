...
* 种子知识处理：从法规便携式文档格式（Portable Document Format，PDF）与制度文本中抽取可用的结构化知识片段。
* 任务体系与样本合成：拆分法条解释、法律问答、案情分析、风险拒答等不同任务层。
* 质量控制与偏好增强：通过质量保障（Quality Assurance，QA）、偏好对和风险边界样本稳定监督信号。
* 训练封装与上线验收：把处理后的数据资产整理成可训练、可验证、可交付的成品。

如果按工程顺序阅读，本章对应的是一条完整链路：

**原始法规便携式文档格式（Portable Document Format，PDF） -> 清洗切块 -> 任务设计 -> 指令合成 -> 偏好增强 -> 质量保障（Quality Assurance，QA）质检 -> 训练封装 -> 上线验收**
...
**目标三：建立可审核、可拒收、可版本化的质量保障（Quality Assurance，QA）机制。**
...
因为行业 SFT 的第一步从来不是“想办法多生成数据”，而是**先保证种子层别脏**。只要种子文本里存在大量格式损伤，后面的模板、思维链（Chain-of-Thought，CoT）、偏好和质量保障（Quality Assurance，QA）都会在脏基础上工作，成本只会越来越高。
...
## 12. 思维链（Chain-of-Thought，CoT）显性化：法律推理的表达约束
...
因此，训练封装并不是简单导出一个 JSON Lines（JSONL），而是要确保：
