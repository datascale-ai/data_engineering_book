...
如果你的任务是论文复现，优先使用 D.2 和 D.4。
如果你的任务是项目落地，优先使用 D.3 和 D.5。
如果你的任务是课程或训练营，优先使用 D.4 和 D.6。
...
如果你的目标不是一个实验脚本，而是一篇可发表、可教学、可复现的章节，建议按下面顺序写：
