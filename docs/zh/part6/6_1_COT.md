...
本篇围绕三类新数据展开：推理数据、工具调用数据，以及 Agent 在多轮交互中的状态与记忆数据。和单轮问答不同，这些数据要记录模型怎样拆解问题、调用了哪些工具、交互状态如何变化、记忆何时被写入或更新。质量检查也不能只看答案对不对，还要看过程是否可解释、工具执行是否成功、状态是否前后一致。
...
对于一般的 SFT 数据，团队往往更关注输入是否清晰、输出是否正确、风格是否稳定；而对于数学、逻辑、代码等推理任务，仅仅拥有“问题—答案”对通常是不够的。因为这类任务的关键不只是模型最后说对了什么，更在于它是否沿着一条可验证、可复现、可泛化的推理路径得到这个答案。很多部署中的不稳定现象，表面上看是“偶发答错”，本质上却是训练数据只奖励最终结果，没有约束中间过程，导致模型在不同题型、不同难度、不同上下文下采用了不一致甚至错误的内部策略。

推理数据工程不能简单理解为普通 SFT 的延长线。它需要在 SFT 和合成数据的基础上，把推理轨迹表示、步骤级验证、错误分类、难度分层和过程监督纳入同一条流水线。也就是说，团队要管理的不只是样本数量，还包括题目如何生成、轨迹如何构造、步骤如何校验、错误如何打标、样本如何进入课程。这样训练出来的模型，才更可能学到可迁移的求解习惯，而不是记住零散题目的答案映射。
...
数学题中，模型可能在某一步使用了不合法的移项、约分或定理，但因为题目结构简单，最后仍然得到正确数值。逻辑任务中，模型可能没有展开前提和规则，而是直接套用了熟悉的结论形式。代码修复中，模型也可能没有找到 bug 的根因，只是碰巧改动了一个能让当前测试通过的位置。表面上，这些样本都像是“成功”；从能力学习的角度看，它们更接近高风险捷径。
...
结果监督的局限不只是“看不到中间过程”。更麻烦的是，它会把题意理解错误、条件遗漏、计算错误、规则误用和状态不一致都压缩成同一个失败标签。团队只知道题没做对，却不知道应该补哪类数据、改哪类模板、收紧哪类验证规则，于是容易继续加数据量或调训练参数，难以触及根因。
...
部署环境中最麻烦的情况不是模型一直表现差，而是“有时很像会做，有时突然失稳”。很多系统上线后的问题并非完全没有能力，而是没有形成稳定的过程机制：某些输入能触发合适路径，稍微换个表述就落入粗糙模板。这种忽好忽坏，比稳定偏弱更容易消耗用户信任。
...
长链任务必须强调过程，因为过程本身承载了正确性。每一步都会成为下一步的条件；一旦前面的状态错了，后面的判断通常也会随之偏离。没有过程约束，模型在复杂任务上的表现很难稳定。

### 从“答案导向”转向“过程导向”

对推理数据团队来说，关键转变是从“只管理答案”转向“同时管理过程”。最终答案当然重要，但在数学、逻辑、代码等任务中，答案只是推理链条的终点。若数据只围绕答案构建，模型更容易学习输出命中策略；若把关键中间状态、规则调用和验证结果纳入数据结构，模型才更可能形成稳定的求解习惯。
...
推理数据工程首先要确定过程的表示形式。CoT、scratchpad、program-of-thought 和 tree-of-thought 不是几种简单的排版方式，它们会影响样本可读性、验证难度、存储成本、标注负担和训练目标。表示方式选得不合适，后续会出现样本难统一、验证难自动化、训练信号不稳定等问题。
...
代码任务更适合采用“问题定位—原因分析—修改方案—代码变更—验证结果”的 schema。它的中间过程和程序状态直接相关。一个高质量代码样本，除了修复前后的代码片段，还应保留触发错误的测试、失败日志、定位依据、候选修复策略和最终验证结果。否则，模型学到的多半只是局部 patch 映射，而不是完整调试逻辑。

逻辑任务的 schema 应突出前提、规则调用、局部结论和冲突检查。逻辑错误常常不是语言不通顺，而是依据缺失、规则适用条件错误或跨步跳推。若不写清“由哪些前提出了哪一步结论”，许多看似自然的推理链条其实并不合法。复杂逻辑任务还可以记录分支讨论、反例测试和假设集变化，方便后续验证。
...
实践中，团队容易从一个极端走向另一个极端：一开始只保留最终答案，后来又无节制地追求更长、更详细的推理文本。轨迹不应按长度判断，而应按有效信息判断。更有训练价值的轨迹，通常能清楚保留关键变换、关键决策、可验证步骤和状态更新。
