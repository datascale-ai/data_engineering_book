...
仅仅将图像的边长拉大了 4.5 倍，**Attention 层的计算量**就显著地膨胀了近 **410 倍**（注意：这里衡量的是 Self-Attention 的二次方复杂度，而非整个模型的 FLOPs；其他层如 FFN 的计算量与序列长度呈线性增长，实际总训练算力涨幅仍然显著但略小于 410 倍）！这样暴戾的显卡显存消耗，将使得再庞大的训练集群也难以负担。因此，图文多模态数据工程最为核心的技术妥协艺术之一，就是不断摸索**“如何对超清大图执行最高效的动态裁切降维与多尺度 Patching 策略”**。
