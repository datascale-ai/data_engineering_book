...
这种问题在传统业务系统中尚可通过加字段权限、补日志、加审批或补脱敏来部分缓解，但在机器学习系统中，风险一旦进入训练过程，就很难通过后置手段完全消除。原因在于，模型本身会把训练数据中的统计规律、长尾样本特征甚至个别极端信息压缩进参数空间中。也就是说，数据暴露不再只是一份表被谁导出的问题，而变成了模型是否已经把不该记住的东西学进去了的问题(Shokri et al. 2017; Zhu et al. 2019; Geiping et al. 2020)。
...
其次是**高维特征的可逆问题**。很多团队以为“去掉姓名、手机号、身份证号”就意味着安全，但这只是移除了直接标识符。对于图像 embedding、文本向量、多模态对齐特征、梯度信息或中间表征而言，攻击者依然可能通过反演方法重建部分原始内容。也就是说，表层字段脱敏并不自动等于模型层隐私安全(Zhu et al. 2019; Geiping et al. 2020)。
...
差分隐私（DP）为这种权衡提供了工程上可量化的表达，即隐私预算。隐私预算通常用 $\epsilon$ 表示，它并不是一笔财务意义上的预算，而是系统允许泄露风险的上界度量。一般来说，$\epsilon$ 越小，保护越强，但准确率和可用性损失通常也越大；$\epsilon$ 越大，噪声越小，模型更有用，但泄露风险也会显著提高(Dwork 2011; Abadi et al. 2016)。

在工程系统中，隐私预算不是一个抽象数学常量，而是一个必须纳入训练轮次、查询次数、实验频率与对外服务配额的治理变量。一个团队若只是会在论文中写出 epsilon，却没有机制去记录每次训练、每次评估、每次对外发布所消耗的预算，那么这个系统在实践中就没有真正管理隐私风险。换言之，预算管理能力本身，就是隐私系统的一部分(Erlingsson et al. 2014; McMahan et al. 2018)。
...
这意味着安全目标从“守住数据库”转向“守住训练过程、协作边界与模型行为”。本章后续所有技术路线，包括 FL、DP、MPC、TEE 与 HE，实际上都服务于这一范式转移：当数据本身无法被自由移动时，系统就必须在训练协议、聚合逻辑与输出边界上重新建立安全性(Kairouz and McMahan 2021; Bagdasaryan et al. 2020)。
...
解决隐私计算问题，业界目前主要演化出五大技术流派。它们并不是互相排斥的，而是在保护对象、计算损耗和适用场景上各有侧重。理解这些技术的关键，不在于死记定义，而在于看清它们分别保护了什么、牺牲了什么，以及它们适合放在系统的哪个阶段(Yang et al. 2019; Kairouz and McMahan 2021)。
...
最典型的是 **FL + DP**。联邦学习解决“原始数据不出域”，差分隐私解决“上传更新或模型输出仍可能暴露个体信息”。这种组合非常适合多方联合训练但又必须控制成员推断风险的场景。第二类典型组合是 **FL + HE**。客户端在本地训练后，把加密后的更新上传给中心端聚合，中心看不到明文梯度，从而降低中心侧的观察风险。第三类是 **TEE + FL**，把聚合逻辑放进可信执行环境，减少云侧或宿主系统窥探训练中间结果的可能性。对于金融名单求交、联合反欺诈等问题，常见组合是 **MPC + PSI**，其重点不是训练大模型，而是安全地完成多方集合运算与联合统计(Bonawitz et al. 2017; Kairouz and McMahan 2021)。
...
联邦学习并不只是“把数据留在本地”这么简单，它本质上是一种重新组织训练控制权的机制。在传统集中式训练中，数据被拉到统一环境，训练者掌握所有样本；而在联邦训练中，训练逻辑必须接受一个事实：参与方的数据域彼此分离、网络状态不同、资源能力不同、信任假设也不同。模型要想学习，只能通过跨域同步参数或梯度，而不是跨域同步原始数据(McMahan et al. 2017; Yang et al. 2019)。
...
联邦学习中最经典的算法是 FedAvg。其直觉非常简单：各客户端在本地训练若干步后，把模型参数更新上传，由中心端按照样本量或权重进行平均，再形成新的全局模型。FedAvg 之所以广泛应用，是因为它结构简单、易实现，并且在很多中等复杂度场景下具有不错的收敛表现(McMahan et al. 2017)。
...
联邦环境下，各节点数据通常并非独立同分布。不同机构的用户结构、设备来源、地区属性、标注口径和数据规模可能截然不同。这会导致全局模型收敛变慢、个别大节点主导训练方向、少数节点长期得不到收益，甚至出现全局模型在平均意义上可接受，但对关键参与方完全不好用的情况(Zhao et al. 2018; Li et al. 2020)。

非 IID 问题决定了联邦训练不是“数据换个地方放”那么简单，而是带有强业务异构性的协作训练。对医疗、金融、政务等场景而言，这种异构性往往比算法本身更难处理，因为它来自机构本身的现实差异，而不是一个可以轻易被代码抹平的技术变量。

#### （4）联邦学习的真实边界

联邦学习的核心承诺是“原始数据原则上不出域”，但这并不等于“绝对隐私安全”。若上传的梯度、参数更新、训练日志或中间指标可被分析，攻击者仍然可能推断本地数据特征。因此，联邦学习通常是隐私系统的“基座”，而不是完整答案。只要系统仍然需要跨域交换某种可分析的信息，就必须继续考虑差分隐私、安全聚合、鲁棒聚合和审计策略(Bonawitz et al. 2017; Geiping et al. 2020)。

---

### 28.2.4 差分隐私（DP）深挖

差分隐私的核心目标，不是把数据变成“完全看不见”，而是让单个样本是否参与系统，不会显著影响输出结果。换言之，攻击者即便掌握大量背景知识，也很难依据模型输出、统计结果或训练过程中的某些可见信号，判断某个个体是否出现在训练数据中(Dwork 2011)。
...
从部署位置看，差分隐私大致可分为 Local DP 和 Central DP。Local DP 要求每个用户或客户端在数据离开本地前就自行加噪，因此信任假设最弱，但由于噪声更早进入系统，精度损失通常更明显。Central DP 则是在中心侧统一加噪，模型质量通常更好，但前提是你必须相信中心能够正确执行预算管理与噪声注入(Erlingsson et al. 2014; Abadi et al. 2016)。

在联邦场景中，两种方式都可能出现。若组织对中心端缺乏足够信任，会更偏向 Local DP；若组织能接受中心端承担更强的治理责任，则可能采用 Central DP，以换取更好的模型效果。

#### （3）训练中的 DP-SGD 思路

在模型训练中，差分隐私最常见的思路是 DP-SGD。其核心步骤通常包括：先对单样本梯度做裁剪，限制每个样本对整体更新的最大影响；再对聚合后的梯度注入特定分布的噪声；最后在训练过程中累计和记录隐私预算的消耗。这个流程体现了 DP 的工程本质：不是“随便往梯度里加点噪声”，而是先限制个体影响边界，再在这个可控边界上进行噪声注入，从而使风险度量具有可解释性(Abadi et al. 2016; McMahan et al. 2018)。
...
安全多方计算更适合结构相对清晰、计算规则明确、参与方数量有限的场景，例如联合求交、联合统计、联合风控评分等。它的核心价值在于：各方在不暴露自身原始输入的前提下，协同完成某种计算，并只得到允许可见的结果(Mohassel and Zhang 2017; Yang et al. 2019)。
...
同态加密最大的吸引力在于“密文可计算”。也就是说，数据即便以加密形式存在，系统仍然可以在不解密的前提下对其进行一定形式的运算，最终解密后获得与明文运算一致的结果。这个特性使 HE 在理论上具有非常强的隐私保护能力，因为计算节点不需要看到明文(Gilad-Bachrach et al. 2016)。
...
TEE 的逻辑是“相信硬件飞地，而不是相信宿主机环境”。它通过 CPU 或硬件平台提供的受保护执行空间，让敏感代码和数据在隔离的环境中运行，即便宿主系统、管理员或云厂商运维具有较高权限，也无法轻易查看其中的明文数据与运行时状态(Tramèr and Boneh 2019)。
...
联邦学习并非只有 FedAvg 一个算法。不同的数据分布特征和系统条件下，需要选择不同的聚合策略。

**表 28-1：主流联邦学习算法对比**

| 算法 | 核心思想 | 适用场景 | 对非 IID 的鲁棒性 | 通信效率 | 实现复杂度 |
|------|---------|---------|-----------------|---------|-----------|
| FedAvg | 客户端本地多步 SGD，服务器加权平均参数 | 数据分布基本均匀、客户端数量较多 | 低：非 IID 严重时收敛慢或漂移 | 高：所需轮次少 | 低：最简单的基线 |
| FedProx | 在本地损失函数中添加近端项，限制参数偏离全局模型 | 数据异构性强、各客户端数据量差异大 | 中高：近端项缓解 client drift | 中：每轮近端计算增加少量开销 | 中：需调整近端参数 μ |
| SCAFFOLD | 引入控制变量(variance reduction)纠正客户端更新方向 | 高度非 IID、各客户端数据分布差异极大 | 高：显式估计并纠正更新方向偏差 | 低：每轮需额外传递控制变量 | 高：需维护全局和本地控制变量 |
| FedNova | 按各客户端实际本地工作量归一化更新，消除步数差异影响 | 客户端异构（计算能力、数据量差异大） | 中：消除更新步数差异带来的偏差 | 中：归一化计算增加少量开销 | 中：需追踪各客户端的工作量 |

**选型决策原则**：

1. **数据分布已知均匀时**（如各分公司的用户画像相似），FedAvg 即足够，无需增加复杂度。
2. **数据异构明显但客户端数量多时**（如跨区域的手机用户数据），FedProx 是平衡复杂度和效果的首选。
3. **数据异构严重且客户端数量有限时**（如跨医院的医疗数据，只有 5-10 家机构），SCAFFOLD 的控制变量机制能显著改善收敛质量。
4. **客户端计算能力差异极大时**（如部分节点是高性能 GPU 服务器、部分节点是移动设备），FedNova 的归一化策略防止慢节点主导训练方向。

****

### 28.2.9 隐私保护技术的工程选型矩阵

在联邦学习基础上叠加差分隐私、安全多方计算或同态加密时，团队面临的是"技术组合"的选型问题。下表从工程落地角度给出每种组合的适用场景和关键权衡。

**表 28-2：隐私保护技术工程选型矩阵**

| 场景 | 推荐技术组合 | 精度影响 | 时延影响 | 运维复杂度 | 不推荐的替代方案 |
|------|------------|---------|---------|-----------|---------------|
| 跨机构联合训练（医疗/金融） | FL + DP（中央DP） | 中：ε=8 时精度下降 1-3% | 低：DP 本身增加少量计算 | 中：需管理隐私预算 | HE：计算开销过大不实用 |
| 跨机构联合训练（高合规要求） | FL + DP + Secure Aggregation | 中高：加密+噪声双重损耗 | 中：Secure Agg 增加通信轮次 | 高：需密钥管理和节点验证 | 纯 MPC：不适合模型训练 |
| 安全推理（在线服务毫秒级） | TEE + 轻量 DP | 低：TEE 无精度损耗 | 低：TEE 接近明文性能 | 中高：依赖特定硬件 | HE：推理时延无法满足毫秒级要求 |
| 多方安全求交（PSI） | MPC + 不经意传输 | 无：精确求交 | 中：千万级记录约 30-60 分钟 | 中：需高效的 PSI 协议实现 | FL：不适用于集合运算场景 |
| 联合统计分析（非训练场景） | MPC + DP | 低中：DP 噪声影响聚合精度 | 高：MPC 通信轮次多 | 高：需同时管理 MPC 和 DP 参数 | FL：FL 是为训练设计的 |
| 数据发布/统计查询 | DP（Local DP 或 Central DP） | 中高：Local DP 精度损失更大 | 低：加噪计算开销小 | 低：只需管理 ε 预算 | MPC：大材小用，时延不划算 |

**选型的三个黄金问题**：

无论场景如何变化，团队在做隐私技术选型时都应先回答以下三个问题：

**问题一：保护什么？** 是保护原始数据不出域（选 FL/MPC/TEE），还是保护个体不可识别（选 DP），还是两者都需要（组合方案）。这个问题的答案直接决定技术路线的大方向。

**问题二：谁能看到什么？** 攻击者模型是什么？是"诚实但好奇"的服务端（Central DP 够用），是"恶意的外部攻击者"（需要 TEE 或 HE），还是"不透明的第三方平台"（需要 Local DP 或 MPC）？攻击者模型的强度决定了防护强度。

**问题三：系统能承受什么代价？** 精度下降 1% 还是 10%？推理时延增加 10ms 还是 10s？运维团队能管理复杂的密钥轮换和节点认证吗？这三个代价约束决定了"理论上正确"的方案中哪些在实际中可行。

---

## 28.3 选型与系统代价评估

引入隐私增强技术，本质上是用系统性能、工程复杂度和组织协作成本，去换取更强的合规可通过性与更低的泄露风险。架构师必须把这些代价显式讲清楚，而不是把隐私方案包装成“零成本升级”。在真实项目中，很多隐私技术失败并不是因为理念错误，而是因为没有人提前告诉业务方：你要为此付出多少算力、多少时延、多少带宽、多少调试成本，以及多少组织协调成本(Kairouz and McMahan 2021)。
...
### 28.3.6 隐私保护系统的成本效益评估框架

隐私保护不是"不计成本的安全"，团队需要建立系统的成本效益评估框架来指导技术选型。

**表 28-3：隐私保护技术的成本效益评估框架**

| 维度 | 评估指标 | FL | DP | MPC | HE | TEE |
|------|---------|----|----|-----|----|-----|
| 精度成本 | 相比明文基线精度下降 | ≤3% | ≤5%(ε=8) | 0% | 0% | 0% |
| 时延成本 | 推理时延增加倍数 | 1x | 1.1x | 100-1000x | 1000-10000x | 1.5x |
| 带宽成本 | 训练通信量增加倍数 | 2-10x | 1x | 10-100x | 10-100x | 1x |
| 运维成本 | 新增团队技能要求 | 中 | 低 | 高 | 高 | 中高 |
| 审计成本 | 可审计性 | 高 | 高 | 中 | 低 | 中 |
| 合规价值 | 满足的合规场景 | 跨域联合训练 | 数据发布、统计查询 | 安全求交、联合计算 | 安全推理 | 安全聚合、敏感处理 |

**成本效益决策流程**：

1. 明确数据保护目标是"不出域"还是"不可识别"还是两者都要。
2. 评估业务系统对精度、时延和带宽的可承受底线。
3. 在满足底线约束的技术方案中，选择合规价值最高的方案。
4. 如果没有任何方案满足所有底线约束，回到第一步重新评估——保护目标是否可以适当放宽？系统约束是否可以调整？
...
横向联邦适用于各参与方特征空间基本一致，但用户群体不同的情况。例如两家区域银行都拥有类似的客户特征字段，但服务的是不同地区人群；又如不同城市的医院拥有相似结构的病例字段，但患者样本不重叠。这类场景中，各方的“列”基本一致，而“行”不同，因此更适合把样本层面的协作能力联合起来，以缓解单一机构样本量不足的问题(Yang et al. 2019)。

横向联邦最大的优势是结构直观，容易理解与部署。但它并不自动意味着训练效果一定好，因为不同机构的用户结构、标签比例与行为模式依然可能存在显著偏差。

### 28.4.2 纵向联邦（Vertical FL）

纵向联邦适用于参与方拥有相似用户群体，但掌握不同特征维度的情况。例如银行掌握用户财务特征，电商掌握消费偏好特征，二者希望在不直接交换原始字段的前提下建立联合风控模型。此时问题的关键不是样本数量，而是特征维度在不同参与方之间的互补性(Yang et al. 2019)。
...
对于大模型场景，越来越多系统采用联邦微调而不是联邦全参数训练。原因非常现实：全量参数训练通信太重、代价太高、隐私暴露面也更大。联邦微调通常会结合 LoRA、Adapter、Prefix Tuning 等 PEFT 方法，仅在各机构间交换较小规模的适配层参数，而不是整个基础模型的全部权重(Hu et al. 2022; Kuang et al. 2024)。
...
联邦训练若要真正成立，必须回答一个问题：聚合器是否能看到每个客户端的单独更新？如果答案是“能”，那么联邦学习虽然阻止了原始数据出域，但仍然可能把梯度泄露风险暴露给中心端。安全聚合的目标，就是让中心端只能看到聚合后的结果，而看不到单个客户端的原始更新(Bonawitz et al. 2017)。
...
### 28.4.6 跨境场景的隐私技术部署方案

跨境数据处理是隐私保护技术最具挑战性的应用场景之一。数据跨越不同法域时，不仅面临法规冲突（如 GDPR 的"充分性认定"与 PIPL 的"安全评估"要求的差异），还需要在技术层面确保数据在传输、存储和处理各个环节的保护。
...
**方案一：安全评估路径的技术准备**。对于需要申报安全评估的出境场景（处理 100 万人以上个人信息的企业），技术团队需要提前准备：出境数据字段清单与分级标注、数据映射（哪些数据流向哪些境外系统）、差分隐私或匿名化处理的参数配置、以及出境数据接收方的保护能力证明。安全评估通过后，技术方案应保持稳定，任何出境数据字段的增减或处理方式的变更都应重新触发评估。

**方案二：标准合同路径的技术实现**。对于标准合同路径，技术侧的核心要求是"最小化出境"——只出境绝对必要的数据字段，且出境前完成脱敏或匿名化处理。工程实现上，应在出境网关上配置字段级过滤规则，确保出境数据不包含超出合同约定的字段。出境网关还应记录每次出境的完整日志（出境时间、字段列表、记录数、出境目的、接收方）并定期与标准合同约定的范围进行对照审计。

**方案三：认证路径的技术要求**。对于通过专业机构认证的路径，技术侧需要满足认证机构的安全技术标准，通常包括：数据加密传输（TLS 1.3+）、存储加密（AES-256）、访问控制（基于角色的最小权限）、审计日志（至少保存 3 年）、以及事件响应机制（数据泄露 24 小时内可追溯）。认证的有效期通常为 3 年，期间技术方案的重大变更应通知认证机构。

**跨境场景的隐私技术组合建议**：

| 跨境场景 | 推荐技术组合 | 核心目的 | 合规路径 |
|---------|------------|---------|---------|
| 跨国企业内部 HR 数据共享 | FL + 加密传输 | 员工数据不出本地，仅共享训练结果 | 标准合同 / BCR |
| 跨境市场分析（聚合统计） | DP + 安全聚合 | 保证聚合结果无法反推个体 | 安全评估 / 标准合同 |
| 跨境风控联合建模 | FL + Secure Agg + DP | 联合训练风控模型，数据不出域 | 安全评估 |
| 跨境客服系统（实时） | TEE + 字段脱敏 | 实时处理但敏感字段不出境 | 标准合同 |
| 跨境研发协作（测试数据） | 匿名化 + MPC-PSI | 测试数据匿名化后通过 MPC 求交 | 认证 / 标准合同 |

### 28.4.7 隐私保护系统的验证与测试

隐私保护系统不能"部署即信任"，必须通过系统化的验证和测试来证明其保护效果符合设计目标。

**验证的三个层面**：

**第一层：功能验证**——证明隐私保护机制被正确实现。例如：差分隐私的加噪逻辑是否实现正确？联邦学习的参数聚合是否按照协议执行？安全多方计算的秘密分享是否正确还原？功能验证通常通过单元测试和集成测试完成，测试用例应包括边界条件（如噪声参数为零时输出与明文一致、隐私预算耗尽时不再响应查询）。

**第二层：安全验证**——证明隐私保护机制在攻击下依然有效。例如：能否通过梯度重建攻击恢复训练数据？能否通过成员推断攻击判断某个个体是否在训练集中？安全验证通常通过"红队测试"完成——模拟攻击者尝试突破隐私保护措施。每次红队测试应记录攻击向量、成功率和保护机制的实际防护效果。

**第三层：效用验证**——证明隐私保护机制在添加保护措施后，系统仍然保留足够的业务效用。例如：差分隐私 ε=8 时模型精度损失是否在可接受范围内？联邦学习在非 IID 数据上的收敛速度是否满足业务需求？效用验证是隐私保护系统能否真正上线的前提——一个"绝对安全但毫无用处"的系统在工程上是失败的。

**隐私测试的自动化框架**：

```python
# 伪代码：隐私保护系统的自动验证框架
def privacy_system_validation(system, test_suite):
    results = {}
    # 1. 功能验证
    for case in test_suite.functional_cases:
        output = system.run(case.input)
        assert output.meets_spec(case.expected)
        results[f"functional_{case.id}"] = "PASS"
    
    # 2. 安全验证（红队测试）
    for attack in test_suite.red_team_attacks:
        attack_result = attack.launch_against(system)
        if attack_result.success_rate > attack.threshold:
            results[f"security_{attack.id}"] = "FAIL - enhancement needed"
        else:
            results[f"security_{attack.id}"] = "PASS"
    
    # 3. 效用验证
    utility_score = system.evaluate_utility(test_suite.benchmark)
    if utility_score < test_suite.utility_threshold:
        results["utility"] = f"WARN - score {utility_score} below threshold"
    else:
        results["utility"] = f"PASS - score {utility_score}"
...
差分隐私的核心资源是"隐私预算 ε"。与 CPU 和内存不同，隐私预算是一种不可再生的消耗性资源。系统必须对隐私预算进行精确的监控和管理：

1. **预算分配**：为每个查询或训练任务预先分配 ε 预算。不同任务根据其业务重要性和数据敏感度获得不同的预算配额。
2. **消耗跟踪**：实时跟踪每个数据集的 ε 消耗总量。当消耗接近预算上限时发出预警。
3. **预算重置**：在数据刷新、模型重训练或合规周期重置时，评估是否允许重置隐私预算。重置应经过审批，并在审计日志中记录重置原因和批准人。
4. **自适应预算调整**：当某个查询的效用显著高于预期时，系统可以自动降低该查询的 ε 分配（减少噪声，提高精度），将节省的预算留给后续更重要的查询。

---

## 28.5 联邦学习中的攻击与防御

一个成熟的隐私章节，不能只讲技术方案，还必须讲攻击面。否则读者很容易误以为“数据不出域”就已经足够安全。事实上，联邦环境中的攻击面并不比集中式训练少，只是攻击路径发生了变化。攻击者不一定要拿到原始数据库，也可以通过观察梯度、参与训练、操纵更新或分析模型输出来实施攻击(Shokri et al. 2017; Bagdasaryan et al. 2020; Geiping et al. 2020)。
...
联邦学习并不会天然消除这类风险。因为全局模型仍然可能对训练样本表现出更高的置信度、更稳定的预测模式或特殊的输出行为。攻击者可以利用这些差异，推断某个样本是否曾参与训练(Shokri et al. 2017)。

### 28.5.2 梯度反演攻击（Gradient Inversion）

梯度反演攻击说明了联邦学习的另一个核心风险：即便原始数据不出域，上传的梯度或参数更新本身也可能包含足够多的信息，使攻击者能够恢复原始训练样本或其近似特征。对于图像任务，攻击者可能重建出样本轮廓；对于文本任务，可能恢复关键词、句式甚至部分敏感片段(Zhu et al. 2019; Geiping et al. 2020)。
...
在联邦环境中，攻击者不一定要偷数据，还可以通过恶意客户端上传被操纵的模型更新，从而污染全局模型。若攻击目标是整体性能下降，这属于模型投毒；若攻击目标是在特定触发条件下让模型输出错误结果，这通常被称为后门攻击(Bagdasaryan et al. 2020)。
...
联邦系统的防御可以从三个层面展开。第一是**隐私防御**，如差分隐私、梯度裁剪与安全聚合，重点在于降低单个样本和单个客户端更新的可分析性。第二是**鲁棒聚合**，如 Median、Trimmed Mean、Krum 等稳健聚合策略，重点在于减小恶意客户端对全局模型的影响。第三是**异常检测**，即对客户端更新幅度、更新方向、损失变化和分布异常进行监控与拦截(Abadi et al. 2016; Bonawitz et al. 2017; Blanchard et al. 2017)。
...
### 28.5.6 模型投毒攻击（Model Poisoning）

模型投毒攻击是联邦学习特有的安全威胁。在集中式训练中，攻击者需要操纵训练数据才能影响模型；在联邦学习中，攻击者只需要操纵自己本地节点的更新，就能影响全局模型。攻击者可以上传恶意参数更新，导致全局模型在某些特定输入上产生预期的错误输出（后门攻击），或者完全破坏全局模型的收敛（无目标攻击）(Bagdasaryan et al. 2020; Bhagoji et al. 2019)。

**防御策略对比**：

| 防御策略 | 核心原理 | 对非 IID 鲁棒性 | 计算开销 | 实现难度 |
|---------|---------|---------------|---------|---------|
| Krum | 选取与其他更新最"接近"的更新，排除异常值 | 低：非 IID 下"接近"的定义失效 | 低 | 低 |
| Trimmed Mean | 对参数逐维度排序，去掉最大和最小部分后平均 | 中：受异常值比例影响 | 中 | 中 |
| Median | 对参数逐维度取中位数替代均值 | 中高：对异常值更鲁棒 | 中 | 低 |
| FoolsGold | 根据客户端更新的相似度历史识别共谋攻击 | 中：需建立历史行为基线 | 中高 | 高 |
### 28.5.7 联邦学习的实用防御组合

没有任何单一防御策略能应对所有攻击。实际部署时，推荐以下组合策略：

**基线防御组合**（适用于大多数场景）：差分隐私 SGD（限制单客户端影响）+ Trimmed Mean 聚合（过滤异常更新）+ 梯度裁剪（约束更新幅度）。这套组合可以抵御大部分成员推断攻击和部分模型投毒攻击。

**增强防御组合**（适用于高安全场景）：在基线组合基础上，增加 Secure Aggregation（防止中心端窥视梯度）+ 客户端验证（验证参与方的身份和训练过程的合法性）+ 历史行为基线异常检测（检测客户端的更新模式是否发生突变）。

**审计防御**（适用于事后追溯）：定期对聚合后的全局模型执行后门检测（检查模型在特定触发模式下的输出是否异常）+ 对客户端更新进行随机抽检（验证客户端是否真的在执行训练而非上传恶意参数）。

### 28.5.8 联邦学习的行业落地案例

**案例一：跨医院的医疗影像模型训练**。5 家三甲医院联合训练肺结节检测模型。每家的影像数据不出院区，通过联邦学习共享模型参数。面临的挑战是数据分布差异极大（不同医院的设备型号、扫描参数、患者群体不同），FedAvg 收敛缓慢。解决方案：采用 FedProx 算法 + 各医院独立的 BatchNorm 层，收敛轮次从 FedAvg 的 500 轮降至 200 轮，模型精度与集中式训练的差距从 5% 缩小到 1.5%。

**案例二：金融机构的联合反欺诈模型**。10 家银行联合训练反欺诈模型，需要共享欺诈交易的特征模式但不暴露具体交易数据。采用的方案是 FL + Secure Aggregation + DP，确保聚合器无法看到任一银行的梯度更新。关键发现：Secure Aggregation 的额外通信开销约为 30%，但在银行的合规审计要求下是可接受的代价。

**案例三：移动设备上的输入法联想模型**。千万级移动设备参与联邦学习训练输入法的联想词模型。采用 FedAvg + Local DP 方案，每个用户在本地加噪后上传更新。核心经验：客户端参与率控制在 5% 以内即可获得足够的训练信号，过高的参与率反而增加带宽压力和收敛波动。

---

## 28.6 联邦系统架构设计

到这一步，章节必须从“技术概念”进入“系统工程”。因为技术路线能否落地，最终取决于系统如何拆分组件、如何组织控制流、如何记录版本与预算，以及如何处理故障和协作变更。换言之，联邦学习不是一个算法函数，而是一套分布式协作系统(Kairouz and McMahan 2021; Kuang et al. 2024)。
...
联邦系统根据参与方的信任关系和网络拓扑，主要有三种部署模式：

**中心化联邦（Client-Server）**：一个中心协调器调度多个客户端。适用于企业-分支机构、或银行联盟等有明确协调方的场景。优点是架构简单、协调方便；缺点是中心存在单点风险和信任集中问题。

**去中心化联邦（Peer-to-Peer）**：没有中心协调器，各参与方直接交换模型更新。适用于参与方对等的联合体（如多家独立医院之间的合作）。优点是消除了中心信任假设；缺点是协调复杂性高、收敛分析更困难。

**分层联邦（Hierarchical FL）**：多个区域聚合器先各聚合区域内客户端，再与全局聚合器同步。适用于超大规模场景（如千万级移动设备），先按地区或运营商聚合，减少全局通信量。

### 28.6.6 联邦系统的性能评估框架

评估联邦系统性能不应只看模型精度，还需要一套专门的联邦性能指标：

| 指标 | 定义 | 评估方式 | 典型目标值 |
|------|------|---------|-----------|
| 收敛轮次 | 达到目标精度所需的最小全局轮次 | 在验证集上追踪精度曲线 | FedAvg 基线 ±20% |
| 客户端参与率 | 每轮实际参与的客户端比例 | 发出邀请数 / 实际响应数 | ≥ 60% |
| 通信效率 | 每轮传输的数据量 | 参数大小 × 参与方数量 | ≤ 100MB/轮（百方级） |
| 异构容错率 | 容忍客户端掉线的比例 | 逐步增加掉线率观察收敛影响 | ≤ 30% 掉线不影响收敛 |
| 隐私预算利用率 | 实际消耗 ε / 总预算 ε | DP 模块的预算追踪 | ≥ 80% 预算用于有效训练 |
...
回到本章开头的医疗案例。这个案例最能说明为什么单一技术往往不够。首先，数据属于 C3 高敏感，天然不允许自由流动；其次，文本、影像、基因属于跨模态高风险组合，单独脱敏并不足以消除对齐后的重识别风险；再次，参与方跨机构且互不完全信任，任何一方都不愿承担集中建湖的责任。医疗联邦学习研究已经展示了跨机构协作在不共享患者原始数据时的可行性与价值(Sheller et al. 2020)。
...
### 28.8.3 隐私保护技术的选型决策案例

**案例一：医疗数据联邦学习——安全与精度的平衡**

某医疗 AI 创业公司与 10 家医院合作开发病理切片诊断模型。每家有 5000-50000 张病理切片，数据总量约 20 万张。法规要求患者数据不得出医院。团队面临三个约束：医院之间网络带宽有限（平均 50Mbps）、模型需要达到 90%+ 的诊断准确率、合规要求必须通过 DPIA 评估。

技术选型过程：
1. 首先确定 FL 为基座，解决"数据不出院"的核心合规要求。
2. 评估 DP 的必要性：病理切片本身不含患者个人信息（已去标识化），但医学影像的独特性使得成员推断仍然可能——如果某患者的切片极为罕见，模型可能对该样本过拟合。因此决定采用较低的 DP 保护（ε=12），在精度和隐私之间取得平衡。
3. 网络带宽方面，采用梯度压缩（只上传 top-1% 的重要梯度），将每轮的通信量从 200MB 降至 2MB。
4. 非 IID 问题：各医院的患者群体不同（不同年龄分布、不同疾病谱），采用 FedProx 替代 FedAvg。

最终结果：模型精度 91.2%（集中式基线 93.5%，差距 2.3%），ε=12 的 DP 保护，每轮训练时间约 45 分钟（100 家医院的 FedAvg 基线为 30 分钟）。

**案例二：金融风控中的 PSI + MPC 联合查询**

某金融科技公司需要与多家银行联合查询黑名单，但各家都不愿意暴露自己的黑名单记录。核心需求是"安全求交"——找出同时出现在多家黑名单中的实体，但各家不能知道其他家的完整黑名单。

技术选型：采用 PSI（隐私集合求交）+ MPC 的组合方案。PSI 用于高效找到共同实体，MPC 用于在共同实体上执行联合统计。

关键工程经验：
1. PSI 协议选择：千万级记录规模下，基于不经意传输的 PSI 协议比基于公钥加密的 PSI 快 10 倍以上。
2. 预处理优化：黑名单数据在参与 PSI 前先经过哈希和布隆过滤器预处理，将参与求交的数据量减少 60%。
3. 审计要求：PSI 流程的每一步生成可验证的证明，确保各家可以验证对方没有在求交过程中收集额外的信息。
...
隐私保护系统需要向监管机构和审计方证明"我确实做到了我承诺的保护"。这一需求催生了隐私审计自动化——将隐私保护的操作痕迹转化为可验证、可追溯、不可篡改的证据链。

**证据链的五层结构**：

1. **策略层证据**：隐私保护策略的版本化记录。包括差分隐私的 ε 预算分配方案、联邦学习的聚合策略配置、数据脱敏规则的版本历史。策略变更应有审批记录，审批记录链接到具体的 RoPA 和 DPIA 报告。
2. **执行层证据**：每次隐私保护操作的具体执行日志。包括差分隐私加噪时的噪声参数和随机种子（加密存储）、联邦学习聚合时参与方的身份和梯度校验结果、MPC 协议运行时的通信日志和验证证明。
3. **验证层证据**：隐私保护效果的独立验证记录。包括成员推断攻击的定期测试结果、梯度反演攻击的防御效果验证、模型投毒检测的扫描报告。验证应由独立于开发团队的第三方或自动化工具执行。
4. **异常层证据**：隐私保护事件（或接近事件）的完整记录。包括隐私预算使用超过 80% 的预警、异常更新的检测记录、客户端行为突变的标记事件。异常事件应包含响应时间线和处置结果。
5. **审计层证据**：定期合规审计的报告和整改记录。包括季度隐私保护审计报告、审计发现项的闭环记录、外部审计或认证的评估结果。

**证据链的自动化要求**：

自动化的隐私审计证据链应满足以下要求：

- 不可篡改性：所有审计日志写入 append-only 存储，历史记录不支持修改或删除。
- 可验证性：第三方审计员可以通过独立计算验证隐私保护参数是否被正确执行。
- 可查询性：审计接口应支持按时间、按数据域、按操作类型、按用户等多维度检索。
- 长期保存：审计日志的保存期限应符合法规要求（PIPL 要求至少保存 3 年，GDPR 要求覆盖处理活动的整个周期）。

| 差分隐私 SGD | 通过加噪天然限制单一客户端的影响 | 高：防御与数据分布无关 | 低 | 低 |
...
**误区一：认为"最强的技术就是最好的技术"**。在同态加密和安全多方计算等"强隐私"技术上，团队往往高估了实际需要的保护强度，低估了精度和时延代价。正确做法：从实际威胁模型出发，选择"够用"而非"最强"的方案。对于大多数场景，FL + DP 的组合已经足够。

**误区二：忽视了运维复杂度**。隐私增强技术的运维成本往往被严重低估。密钥管理、节点认证、安全聚合的故障恢复、隐私预算的持续监控——这些运维需求在 PoC 阶段不明显，在规模化部署时会急剧放大。一个在实验室运行良好的方案，在生产环境中可能因为运维复杂度而崩溃。

**误区三：认为"隐私技术可以替代合规流程"**。隐私增强技术是合规的工具，但不是合规的全部。部署了联邦学习不等于自动满足 GDPR 的合规要求。合规流程（RoPA、DPIA、审计）仍然是必要的，隐私技术只是让这些流程的"数据不泄露"承诺更加可信。

**误区四：低估了内部沟通成本**。隐私保护技术的部署需要算法团队、平台团队、安全团队和法务团队的高度协同。算法的参数调优影响精度的同时影响合规声明，平台的架构设计影响性能的同时影响审计能力。这些跨团队的沟通成本往往是项目延迟的最大原因，而非技术本身。

### 28.10.2 隐私保护技术的未来趋势

几个正在影响隐私保护技术发展的重要趋势值得关注：

**联邦学习与基础模型的结合**。基础模型（LLM、VLM）的参数量达到数十亿甚至数千亿级别，传统的 FedAvg 方式在带宽和通信上几乎不可行。新的研究方向包括：只共享 LoRA 适配器参数而非完整模型参数、在联邦框架下实现指令微调和 RLHF、以及通过联邦方式将私有数据注入 Prompt 上下文而非模型参数。

**差分隐私的行业标准化**。差分隐私正在从学术概念走向行业标准。Apple、Google、Microsoft 等企业已经在实际产品中部署 DP，而 NIST 正在推动差分隐私的标准化工作。标准化将带来工具链的成熟和部署门槛的降低，使 DP 不再需要"专家调参"才能使用。

**隐私保护与数据工程 Agent 的结合**。Agent 系统可以自动执行隐私保护策略的配置和监控——根据数据敏感度自动选择 DP 参数、监控隐私预算消耗、检测异常访问模式、生成合规审计报告。这种"隐私 Agent"将大幅降低隐私技术的运维复杂度。

### 28.10.3 给团队的最终建议

隐私保护技术的选型和落地不是一次性的技术决策，而是持续的组织学习过程。以下是给数据工程团队的最终建议：

起步时不追求完美。从最简单的方案开始（制度治理 + 基础脱敏），在运行中逐步发现问题并升级技术方案。一个运行良好的 L1 制度治理方案，远胜于一个部署了一半就搁置的 L3 隐私计算方案。

建立可退出的架构。隐私保护技术仍在快速发展，今天的"最佳方案"可能在 2-3 年后被更好的方案取代。架构设计时应保持可替换性——隐私引擎是插拔式的，与业务逻辑解耦。技术栈的更换不应需要重写整个系统。

持续验证而非一次部署。隐私保护效果需要持续验证——随着数据分布变化、模型更新和参与方变化，隐私保护的有效性可能退化。定期（每季度）的隐私红队测试应该成为团队的常规实践。
...
Tramèr F, Boneh D (2019) Slalom: Fast, Verifiable and Private Execution of Neural Networks in Trusted Hardware. International Conference on Learning Representations.
