...
## 摘要
本章聚焦大模型训练中易被忽视却极具性能影响的环节——输入数据管道（Input Pipeline）。章节通过一个因数据 I/O 导致 GPU 算力闲置的工程案例，揭示了数据加载效率对训练成本的决定性影响。首先，对比了 BPE、WordPiece 和 SentencePiece 等主流分词算法的原理与领域适配策略；随后，详细探讨了 JSONL、Parquet、MDS 等数据存储格式在顺序读取与随机访问中的性能权衡。进一步，本章引入了消除 Padding 算力损耗的 Packing 策略，以及温度采样与课程学习等异构数据混采技术。最后，提供了一套针对 DataLoader 的分布式读取方案与吞吐瓶颈诊断三步法，并附录了生产级预训练任务启动前的检查清单。

**关键词**：输入管道；数据分词；序列化；序列 Packing；课程学习；吞吐瓶颈；内存映射

**学习目标**
- 理解数据输入管道（I/O）对 GPU 算力利用率及训练成本的影响。
- 掌握主流分词算法的差异及垂直领域词表扩充的方法。
- 对比不同数据序列化格式，掌握 MDS 或 Memmap 的工程实践。
- 了解序列 Packing、温度采样及课程学习等高级数据混采技术。
- 掌握分布式数据加载中的吞吐瓶颈排查与修复流程。

## 6.1 开篇：输入管道对训练性能的制约

### 6.1.1 场景引入：数据加载瓶颈导致的算力闲置

某团队在启动 13B 参数模型的预训练时，申请了 64 张 A100 GPU 组成的计算集群。训练在第 2 小时开始出现异常：监控显示 GPU 利用率稳定在 38% 左右，而不是预期的 85% 以上。初步排查认为是模型配置问题，直到通过系统监控发现磁盘 I/O 已经满载：读取速度维持在磁盘上限的 100%，但 DataLoader 的吞吐率依然无法满足 GPU 的计算需求。

根本原因在于：团队将清洗好的语料存放在普通硬盘阵列上，且采用压缩的 JSONL 格式存储。这要求 DataLoader 在运行时实时解压和分词，导致 CPU 和磁盘双双成为瓶颈。最终，团队将训练暂停，重新对所有数据进行离线分词，并将其序列化为 MDS 格式（Mosaic Data Shard），同时迁移到高速固态存储上，才将 GPU 利用率恢复至常规水平。

这个案例说明了本章的核心命题：**数据输入管道（Input Pipeline）的效率，是预训练工程中影响资源利用率的关键环节。**它处于“清洗”与“训练”的交界，常常容易被忽视。
...
在传统的基于词级（Word-level）的旧时代分词器中，如果遇到未被记录在词表中的生僻字或罕见词，模型通常会抛出一个代表未知的 `<UNK>`（Out-of-Vocabulary）占位符。这在医学、法律等专业领域影响显著：一段含有复杂化学式的文本可能会出现大量的 `<UNK>`。而 BPE 和 WordPiece 这种基于 Subword 的方案，在遇到未见过的单词时，会一直向下拆分为更基础的子词甚至单字母/单字节。虽然增加了序列长度，但永远不会出现真正的 OOV 截断，保证了信息的无损传入。
...
**代码清单 6-3：SentencePiece 多语言词表训练配置**
```python
...
**代码清单 6-5：PyTorch DataLoader 与 StreamingDataset 配置**
```python
...
*图6-2：吞吐瓶颈诊断流程图 —— 从 GPU 利用率异常出发，通过三级决策树定位磁盘 I/O 瓶颈、CPU 预处理瓶颈和 PCIe 传输瓶颈，并给出对应的修复方案。（来源：本书自绘）*
...
**代码清单 6-7：分布式场景下的 MosaicML Streaming 读取**
```python
...
*图6-1：LLM 训练输入管道分层架构 —— 从分词、序列化、数据混采、Packing 到 DataLoader GPU 馈送的五阶段完整路径，底部标注了两个最高频的瓶颈风险点（磁盘 I/O 和 CPU↔GPU 传输）。（来源：本书自绘）*
...
本章与第3章（成本治理）的成本视角深度呼应——在 GPU 算力成本极高的预训练任务中，输入管道的工程质量差异可以直接决定数十乃至数百万人民币的算力成本节省。进入下一章，我们将视角从"如何把数据送进模型"转向"如何评价模型用这些数据学到了什么"：**第7章 数据评估、质量闭环与运营迭代**。
