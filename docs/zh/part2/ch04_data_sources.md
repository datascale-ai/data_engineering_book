...
## 摘要
本章系统探讨了大模型预训练数据源的选择、配比、采集工程与合规治理。高质量的预训练数据是大模型能力的上限。章节首先通过一个因采集低质 SEO 站群数据而导致模型表现不佳的案例，说明“源头治理”的重要性。随后，梳理了开放网页、论坛、百科、代码、学术论文等核心数据源的特征及其在不同模型任务中的最佳配比策略。在数据获取环节，重点介绍了分布式异步采集架构与针对异构数据（如 WARC 网页记录与 PDF 文档）的高效解析方法，并引入元数据存证链机制。最后，本章提出了白、灰、黑三级名单与许可证自动化分类策略，为在法律合规与数据规模之间取得平衡提供工程化解决方案。

**关键词**：数据源策略；数据采集；版权合规；数据配比；异步爬虫；文档解析；元数据存证

**学习目标**
- 理解数据源质量对大模型预训练能力上限的决定性作用。
- 掌握开放网页、代码、学术论文等核心数据源的特征、合规风险与配比策略。
- 熟悉工业级分布式异步采集架构与针对复杂格式的解析方法。
- 建立数据权属存证与许可证分级管理的合规治理意识。

## 4.1 开篇：预训练语料为何受制于源头质量

### 4.1.1 场景引入：当算力投入未能转化为有效能力

某 AI 研究院的算法团队花费了三个月时间，从公开的网络资源里爬取并积累了超过 2TB 的中文文本数据。按照 Chinchilla 最优配比，这足以支撑一轮 7B 模型的预训练。在启动了长达两周的训练任务之后，团队进行了首次评测：模型在中文阅读理解和数学推理上的表现不如同规模的开源基线模型，且模型频繁输出无实质内容的凑字数段落，以及疑似来自某些论坛的仿网络小说片段。

复盘后，团队发现问题的根源在于：2TB 数据中，有接近 60% 来自 SEO 站群（内容通过自动采集和改写生成的低质量网站），15% 是各类网络小说正文（版权高度存疑），只有不到 25% 是具有实质知识密度的内容（如百科、技术文档、学术摘要等）。模型忠实地学习了训练集的分布，表现出了符合训练数据特征的行为：输出流畅却空洞的文字。

这个案例揭示了预训练数据工程中的核心原则：**源头数据的质量缺陷，难以依靠后续清洗管线完全弥补**。当数据源缺乏足够的信息熵与知识密度时，清洗过程仅能去除明显的格式噪声，而无法注入知识。
...
这条因果关系有一个重要的延伸含义：**预训练数据的质量上限，决定了模型能力的理论上限**。一个在高质量多样化语料上预训练的模型，即使后续 SFT 数据量较少，往往也能通过指令微调达到不错的效果；而一个在劣质偏斜语料上预训练的模型，无论后续投入多少 SFT 资源，都很难突破底层认知能力的缺陷——因为 SFT 改变的是模型的"行为风格"，而不能凭空注入预训练阶段没有学到的知识结构。
...
如果说本章是一份大模型数据工程的“审计清单”，那么数据源地图就是这份清单的核心视图。它帮助工程师在动手采集任何数据之前，先回答一个关键问题：**我们的语料从哪里来，各占多少，质量和法律风险如何？**

![图4-1：预训练数据源分层地图](../../images/part2/pretrain_data_source_map.png)

*图4-1：预训练数据源分层地图 —— 三层分类体系按照处理复杂度、知识密度和许可风险对主流数据来源进行定位，并给出典型的配比参考区间。（来源：本书自绘）*

### 4.2.1 八类核心数据源全景

**开放网页（Open Web）** 是体量最大、处理难度最高的一类来源，以 Common Crawl 为代表。Common Crawl 从 2008 年起持续抓取互联网，每月发布数十亿网页的快照，累计数据量超过 PB 级，是目前几乎所有大规模预训练数据集的上游来源。然而，开放网页的原始数据包含大量冗余——据 FineWeb 项目的统计 (Penedo et al. 2024)，Common Crawl 原始内容中，具备高知识密度的正文内容占比较低，剩余大部分为广告、导航栏、SEO 推广内容及 JavaScript 代码等噪声。这意味着网页数据必须经过严苛的清洗才能使用（详见第5章）。
...
**代码清单 4-1：基于 aiohttp 与 robots.txt 校验的异步爬虫**
```python
...
**网页（HTML/WARC）** 是最常见且需要专业工具处理的格式。Common Crawl 提供了三种数据格式：WARC（原始 HTTP 响应+完整 HTML）、WAT（元数据）和 WET（预提取纯文本）。早期实践中常直接使用 WET 文件以节省解析成本。然而，Common Crawl 的 WET 提取使用的是通用算法，常常保留大量导航栏、页脚、广告文字和代码片段，引入过多噪声。更为严谨的做法是从 WARC 文件出发，使用高精度的正文提取库（如 Trafilatura (Barbaresi 2021)）重新解析。虽然计算开销增加，但可以显著提升后续有效内容的保留率。

**代码清单 4-2：使用 Trafilatura 解析 WARC 格式记录**
```python
import trafilatura
...
*图4-2：数据采集与权属存证流程——从数据源触达到最终归档，每个处理阶段均向"Provenance Ledger（权属账本）"追加元数据记录，形成完整的可审计数据血缘链路。（来源：本书自绘）*
...
**黑名单（Blacklist）**：明确不可用的来源，包括已发生诉讼的来源、robots.txt 明确禁止的网站、以及任何包含明确"禁止用于 AI 训练"声明的数据。技术上，可以通过 URL 域名前缀匹配的方式，在采集管线的入口阶段自动拦截黑名单来源：

**代码清单 4-4：版权黑名单 URL 拦截过滤机制**
```python
# 版权黑名单：在采集入口拦截禁止来源
...
进入下一章，我们将在本章采集到的原始数据基础上，探讨预训练数据工程的关键步骤：**第5章 清洗、去重与去污染**——介绍如何将带有噪声的原始语料转化为可供训练的高质量数据集。源头治理确立了数据的能力上限，而清洗管线则决定了最终能够转化为有效训练特征的真实规模。这两者共同构成了预训练数据工程的质量屏障。
