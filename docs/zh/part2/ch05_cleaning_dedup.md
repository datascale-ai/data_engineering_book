...
## 摘要
本章详细介绍了大模型预训练数据工程中最核心的质量屏障——数据清洗、去重与去污染体系。数据清洗并非简单的规则过滤，而是决定模型泛化能力上限的关键步骤。章节首先通过因重复数据导致模型“复读机”现象的案例，强调精细化去重的重要性。随后，构建了规则过滤、模型打分与人工抽检协同的三级清洗框架，并深入讲解了文本标准化方法。在去重技术方面，对比了精确哈希去重与 MinHash LSH 模糊去重的算法原理与工业实现。针对隐私与评测诚信问题，本章给出了 PII 脱敏的正则与 NER 组合策略，以及基于 N-gram 的基准去污染方案。最后，通过案例复盘清洗过度与遗漏的代价，为不同规模的团队提供了最小可行的清洗组合。

**关键词**：数据清洗；模糊去重；PII 脱敏；基准去污染；困惑度模型评分；文本标准化；质量抽检

**学习目标**
- 理解数据缺陷（重复、低质、污染）在模型训练中的指数放大效应。
- 掌握规则过滤、模型评分与人工验证三级协同的数据清洗框架。
- 深入理解并实现 MinHash LSH 模糊去重与精确去重算法。
- 建立 PII 隐私合规脱敏与基准去污染（Benchmark Decontamination）的工程标准。

## 5.1 开篇：清洗缺陷如何在训练中被放大

### 5.1.1 场景引入：数据重复导致过拟合与“复读机”现象

某团队在完成了第4章的数据采集之后，启动了一个 7B 参数中文基座模型的预训练。训练初期各项指标稳定，但在基准评测时发现：模型在续写任务中会反复生成完全相同的句子；此外，在特定的触发词下，模型甚至能够逐字不差地输出某电商平台的商品描述格式。

这反映了典型的数据重复导致的过拟合问题。工程师发现，训练数据中包含的大型电商平台商品描述，因不同 URL 路径被爬取了数十次。虽然在数据接入时执行了基于 URL 的精确去重，但未能识别内容近似的重复记录。

这个案例表明：**数据清洗不仅是剔除无效数据，更是构建训练数据质量上限的系统工程。**

### 5.1.2 清洗投入与训练收益的非线性关系
...
这一发现反映出“数据量大即可”的粗放思维存在局限。在算力资源有限的情况下，**将有限算力用在高质量数据上，优于把同等算力消耗在低质数据上**。因此，数据清洗工程投资的 ROI 往往是整个大模型研发链路中最高的环节之一。

### 5.1.3 上游缺陷如何在训练阶段被指数放大
...
*图5-1：清洗与去污染全景流程图 —— 多阶段质量闸门从原始语料（100%）精炼至最终训练语料（~15-25%），每阶段标注了典型的过滤比例。（来源：本书自绘）*
...
**困惑度过滤（Perplexity Filter）**是目前广泛采用的模型过滤方法。使用 **KenLM (Heafield 2011) n-gram 语言模型**（而非神经网络模型）计算困惑度时，可以将文本质量量化：高质量的新闻和百科文本困惑度通常在 100-300 之间；普通通顺网页文本在 200-500 之间；乱码、机器翻译等低质量内容往往超过 500。值得注意的是，困惑度并非越低越好——困惑度极低（低于 50）的文本可能是高度同质化的样板文本（如格式几乎固定的法律条文或商品说明），也需要额外关注。（注：若改用神经网络参照模型如 LLaMA-7B 计算，相同文本的 PPL 数值会显著缩小至 20–150 区间，详见第7章 §7.3.1，两者不可混用阈值。）
...
**繁简体处理策略**对中文大模型有特别的重要性。训练全参数的多方言中文模型时，繁体字和简体字应当共存；但若目标是一个专注于大陆简体中文的模型，建议对繁体语料进行简体转换（opencc 库可以实现高质量的繁简转换）。需注意：机械的繁简转换会丢失一些台湾、香港等地区特有的词汇和表达，对于涉及这些地区的垂直领域模型，需要谨慎处理，而不应采用单一的无差别简繁转换。
...
无论是精确的哈希去重还是基于 N-gram 的 MinHash LSH，本质上都是在捕捉文本的“字面特征”。但如果同一篇新闻报道被两家媒体用完全不同的词汇改写（如同义词替换、句子结构重组），字面重叠度极低，此时传统的 LSH 将难以发挥作用。为了应对这种情况，**语义去重（Semantic Deduplication）** 引入了 Embedding 模型。

在实现上，通常使用轻量级的 Embedding 模型（如 `BGE-M3` 或 `text2vec`）将每篇文档编码为稠密向量，然后通过向量数据库（如 Milvus 或 FAISS）构建近似最近邻（ANN）索引。若两个文档向量之间的余弦相似度（Cosine Similarity）超过 0.95，即使它们的字面完全不同，也会被识别为高度语义同质化内容并进行去重。
由于计算 Embedding 需要消耗可观的 GPU 推理算力，工业界通常将其作为**去重管线的最后一环**：先用极低成本的哈希去重和 MinHash 筛掉 90% 的冗余，再对剩下的高价值语料进行语义去重，在算力成本与去重精度之间取得平衡。
...
*图5-2：质量过滤漏斗与抽检闭环 —— 左侧漏斗展示每阶段的数据留存率，右侧闭环展示人工抽检如何驱动过滤规则的持续迭代优化。（来源：本书自绘）*
...
| **PII 泄露** | 手机号、身份证、邮箱、API Key | 正则规则 + NER 模型 + 人工抽检 | 上线后隐私事故 | 严格控制，人工复核 |
